...
def run_experiment_with_disjoint_environments(
    ...
    while playing and step < num_timesteps:
        ...
        for partition_idx in active_partitions:
            ...
            llm_decision = "delegate_to_rl"  # Placeholder for the LLM decision
            ...
            try:
                ...
                # Get the partition's region coordinates
                partition_info = env_manager.partitions[partition_idx]
                region_coords = partition_info['region_coords']

                # Extract coordinates (format is (y_start, x_start, y_end, x_end))
                y_start, x_start, y_end, x_end = region_coords

                # Calculate width and height
                width = x_end - x_start + 1  # +1 because coordinates are inclusive
                height = y_end - y_start + 1

                # Extract the subsurface for this specific partition
                try:
                    ...
                    # Extract subsurface for this partition (pygame uses x, y, width, height)
                    subsurface = screen.subsurface((x_start, y_start, width, height))
                    
                    print(f"Extracted subsurface for partition {partition_idx}: "
                        f"region (y:{y_start}-{y_end}, x:{x_start}-{x_end}) -> size ({width}x{height})")
                    
                except ValueError as e:
                    print(f"Error extracting subsurface for partition {partition_idx}: {e}")
                    print(f"Attempted region: (x:{x_start}, y:{y_start}, width:{width}, height:{height})")
                    print(f"Screen size: {screen.get_size()}")
                    # Fallback: use a safe default subsurface
                    subsurface = screen.subsurface((0, 0, min(128, screen_width), min(128, screen_height)))
                # Capture the game state image for this partition
                ...
                if step % LLM_CALL_FREQUENCY == 0 and step > 0:
                    ...
                    if not FORCE_DELEGATE_TO_RL:
                        try:
                            if USE_IMAGE_PROMPT:
                                response = asyncio.run(call_agent_async_master(prompt, game_state_image_small, runner, USER_ID, SESSION_ID))
                            else:
                                response = asyncio.run(call_agent_async_master(prompt, game_state_image=None, runner=runner, USER_ID=USER_ID, SESSION_ID=SESSION_ID))
                ...
                if llm_decision == "delegate_to_rl":
                    ...
                    try:
                        ...
                        print(f"  Batched observation okay")
                        ...
                        print(f"  ERROR getting action from RL model for partition {partition_idx}")
                        action_rl = jnp.array(0)
                        partition_state['actions'].append(action_rl)
                        action_list.append(action_rl)
                elif llm_decision == "delegate_to_llm":
                    # local_map = generate_local_map(env_manager.small_env_timestep)
                    ...
                    action_rl = jnp.array([action_output], dtype=jnp.int32)  # Convert to JAX array
                    action_list.append(action_rl)
                    ...
                    action_rl = jnp.array([-1], dtype=jnp.int32) # Use jnp.array
                    action_list.append(action_rl)
