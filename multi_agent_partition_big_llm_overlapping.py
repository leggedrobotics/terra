...
class DisjointMapEnvironments:
    ...
    def set_partitions(self, partitions):
        ...
        Args:
            partitions: List of partition dicts with 'region_coords', 'start_pos', etc.
        """
    ...
    def _partitions_overlap(self, i: int, j: int) -> bool:
        ...
        # Check for overlap
        return (y1_start <= y2_end and y2_start <= y1_end and 
    ...
    # Simple test sync that should work now
    def test_sync_with_fixed_overlaps(self, partition_states):
        """
        Test synchronization with the fixed overlap detection.
        """
        print(f"\n=== TESTING SYNC WITH FIXED OVERLAPS ===")
        
        # Check if we have any overlaps
        if not self.overlap_regions:
            print("No overlap regions found - cannot sync!")
            return False
        
        print(f"Found {len(self.overlap_regions)} overlap regions")
        
        # Try to sync between each overlapping pair
        for (i, j), overlap_info in self.overlap_regions.items():
            ...
            print(f"\nSyncing between partitions {i} and {j}")
            print(f"  Overlap info: {overlap_info}")
            
            # Get the maps
            map_i = partition_states[i]['timestep'].state.world.traversability_mask.map
            map_j = partition_states[j]['timestep'].state.world.traversability_mask.map
            
            slice_i = overlap_info['partition_i_slice']
            slice_j = overlap_info['partition_j_slice']
            
            print(f"  Map {i} shape: {map_i.shape}, slice: {slice_i}")
            print(f"  Map {j} shape: {map_j.shape}, slice: {slice_j}")
            
            # Extract overlap data
            try:
                overlap_data_i = map_i[slice_i]
                overlap_data_j = map_j[slice_j]
                
                print(f"  Overlap data shapes: {overlap_data_i.shape} vs {overlap_data_j.shape}")
                print(f"  Data matches: {jnp.array_equal(overlap_data_i, overlap_data_j)}")
                
                if not jnp.array_equal(overlap_data_i, overlap_data_j):
                    # Simple copy from i to j for testing
                    updated_map_j = map_j.at[slice_j].set(overlap_data_i)
                    
                    # Update partition j
                    updated_world = self._update_world_map(
                        partition_states[j]['timestep'].state.world, 
                        'traversability_mask', 
                        updated_map_j
                    )
                    updated_state = partition_states[j]['timestep'].state._replace(world=updated_world)
                    updated_timestep = partition_states[j]['timestep']._replace(state=updated_state)
                    partition_states[j]['timestep'] = updated_timestep
                    
                    print(f"  ✓ Copied data from partition {i} to partition {j}")
    ...
    def add_other_agents_as_obstacles(self, partition_states):
        """
        Add other agents as obstacles in each partition's view.
        """
        print(f"\nAdding other agents as obstacles...")
        ...
        for target_partition_idx, target_partition_state in partition_states.items():
            ...
            for other_partition_idx, other_partition_state in partition_states.items():
                ...
                other_agent_state = other_partition_state['timestep'].state.agent.agent_state
                other_pos = other_agent_state.pos_base
                
                # Convert to global coordinates
                other_partition = self.partitions[other_partition_idx]
                other_region_coords = other_partition['region_coords']
                
                global_other_y = float(other_pos[0]) + other_region_coords[0]
                global_other_x = float(other_pos[1]) + other_region_coords[1]
                
                # Check if this agent is visible in the target partition
                if (target_y_start <= global_other_y <= target_y_end and 
                    target_x_start <= global_other_x <= target_x_end):
                    
                    # Convert to local coordinates
                    local_other_y = int(global_other_y - target_y_start)
                    local_other_x = int(global_other_x - target_x_start)
                    
                    if (0 <= local_other_y < traversability.shape[0] and 
                        0 <= local_other_x < traversability.shape[1]):
                        
                        # Add obstacle around the other agent (only in free space)
                        agent_size = 5  # 5x5 footprint
                        half_size = agent_size // 2
                        
                        for dy in range(-half_size, half_size + 1):
                            for dx in range(-half_size, half_size + 1):
                                y, x = local_other_y + dy, local_other_x + dx
                                if (0 <= y < traversability.shape[0] and 
                                    0 <= x < traversability.shape[1]):
                                    # Only mark free space as obstacle
                                    if traversability[y, x] == 0:
                                        traversability = traversability.at[y, x].set(1)
                        
                        agents_added += 1
                        print(f"  Added agent {other_partition_idx} as obstacle in partition {target_partition_idx} at ({local_other_y}, {local_other_x})")
    ...
    def complete_synchronization(self, partition_states):
        """
        Complete synchronization function that handles both terrain and agents.
        Call this after all partitions have stepped.
        """
        ...
        # Step 2: Add other agents as obstacles
        self.add_other_agents_as_obstacles(partition_states)
        
        print("✓ Complete synchronization finished")

    # Updated complete synchronization function
    def complete_synchronization_with_full_agents(self, partition_states):
        ...
        # Option 1: Use calculated footprint with orientation
        # self.add_other_agents_as_full_obstacles(partition_states)
        
        # Option 2: Use existing representation from other partition (RECOMMENDED)
    ...
    def test_complete_sync(self, partition_states):
        """
        Test the complete synchronization system.
        """
        print(f"\n=== TESTING COMPLETE SYNCHRONIZATION ===")
        
        print("BEFORE sync:")
        self.debug_terrain_differences(partition_states)
        
        # Run complete sync
        self.complete_synchronization(partition_states)
        
        print("\nAFTER sync:")
        self.debug_terrain_differences(partition_states)

    def debug_terrain_differences(self, partition_states):
        """
        Debug terrain differences between overlapping partitions.
        """
        for (i, j), overlap_info in self.overlap_regions.items():
            ...
            trav_i = partition_states[i]['timestep'].state.world.traversability_mask.map
            trav_j = partition_states[j]['timestep'].state.world.traversability_mask.map
            
            overlap_i = trav_i[slice_i]
            overlap_j = trav_j[slice_j]
            
            # Count obstacles
            obstacles_i = jnp.sum(overlap_i == 1)
            obstacles_j = jnp.sum(overlap_j == 1)
            
            # Count agents
            agents_i = jnp.sum(overlap_i == -1)
            agents_j = jnp.sum(overlap_j == -1)
            
            print(f"  Overlap {i}<->{j}: Obstacles({obstacles_i},{obstacles_j}), Agents({agents_i},{agents_j}), Match: {jnp.array_equal(overlap_i, overlap_j)}")
    ...
    def _update_global_maps_from_partition(self, partition_idx: int, timestep):
        """Update global maps with changes from a specific partition."""
        partition = self.partitions[partition_idx]
        ...
        small_state = timestep.state
        
        # Maps that can change during gameplay
        changeable_maps = ['dumpability_mask', 'action_map', 'dig_map']
        
        for map_name in changeable_maps:
            if hasattr(small_state.world, map_name):
                small_map = getattr(small_state.world, map_name).map
                # Update the corresponding region in global maps
                self.global_maps[map_name] = self.global_maps[map_name].at[region_slice].set(small_map)

    def _update_overlapping_partitions(self, updated_partition_idx: int, partition_states: dict):
        """
        Update overlapping regions in other partitions when one partition changes.
        FIXED VERSION: Only updates environmental changes, not agent positions.
        """
        if updated_partition_idx not in self.overlap_map:
            return
        
        overlapping_partitions = self.overlap_map[updated_partition_idx]
        if not overlapping_partitions:
            return
            
        print(f"Updating environmental overlaps for partition {updated_partition_idx} -> {list(overlapping_partitions)}")
        
        for other_partition_idx in overlapping_partitions:
            if (other_partition_idx in partition_states and 
                partition_states[other_partition_idx]['status'] == 'active'):
                
                self._update_single_overlap_environmental(updated_partition_idx, other_partition_idx, partition_states)
    def _update_single_overlap(self, source_partition: int, target_partition: int, 
                              partition_states: dict):
        """
        Update the overlapping region from source partition to target partition.
        """
        overlap_key = (source_partition, target_partition)
        ...
        print(f"  Updating overlap: partition {source_partition} -> partition {target_partition}")
        
        # Get overlap region information
        global_slice = overlap_info['global_slice']
        source_slice = overlap_info['partition_i_slice']
        target_slice = overlap_info['partition_j_slice']
        
        # Get current states
        source_state = partition_states[source_partition]['timestep'].state
        target_state = partition_states[target_partition]['timestep'].state
        
        # Maps that need to be synchronized in overlapping regions
        maps_to_sync = ['dumpability_mask', 'action_map', 'dig_map']
        
        updates_made = False
        for map_name in maps_to_sync:
            if hasattr(source_state.world, map_name) and hasattr(target_state.world, map_name):
                source_map = getattr(source_state.world, map_name).map
                target_map = getattr(target_state.world, map_name).map
                
                # Extract data from source overlap region
                source_overlap_data = source_map[source_slice]
                
                # Check if there are actual differences (to avoid unnecessary updates)
                target_overlap_data = target_map[target_slice]
                if not jnp.array_equal(source_overlap_data, target_overlap_data):
                    # Update target overlap region
                    target_map = target_map.at[target_slice].set(source_overlap_data)
                    
                    # Update the target state's map (this is the tricky part)
                    # We need to recreate the world state with updated map
                    ...
                    updates_made = True
                    print(f"    Updated {map_name} in overlap region")
        
        if updates_made:
            print(f"  Overlap update completed: {source_partition} -> {target_partition}")
        else:
            print(f"  No updates needed: {source_partition} -> {target_partition}")
    ...
    # def step_with_overlap_updates(self, partition_idx: int, action, partition_states: dict):
    #     """
    #     ENHANCED VERSION: Step a partition and ensure ALL changes are properly synchronized.
    #     """
    #     # Step the environment normally first
    #     partition_state = partition_states[partition_idx]
    #     current_state = partition_state['timestep'].state
    #     current_env_cfg = partition_state['timestep'].env_cfg
    ...
    #     # Update global maps with changes from this partition
    #     self._update_global_maps_from_partition(partition_idx, new_timestep)
        
    #     # STEP 1: First sync all environmental changes (terrain, digging, dumping)
    #     self._sync_environmental_changes_to_overlapping_partitions(partition_idx, partition_states)
        
    #     # STEP 2: Then update agent positions for all partitions
    #     for target_partition_idx in partition_states.keys():
    #         if partition_states[target_partition_idx]['status'] == 'active':
    #             self._update_traversability_with_other_agents(target_partition_idx, partition_states)
        
    #     return new_timestep

    def _sync_environmental_changes_to_overlapping_partitions(self, updated_partition_idx: int, partition_states: dict):
        """
        Sync ALL environmental changes (including terrain obstacles) to overlapping partitions.
        """
        if updated_partition_idx not in self.overlap_map:
            return
        ...
        print(f"Syncing environmental changes from partition {updated_partition_idx} to {list(overlapping_partitions)}")
        
        for other_partition_idx in overlapping_partitions:
            if (other_partition_idx in partition_states and 
                partition_states[other_partition_idx]['status'] == 'active'):
                
                self._sync_single_overlap_environmental(updated_partition_idx, other_partition_idx, partition_states)

    def _sync_single_overlap_environmental(self, source_partition: int, target_partition: int, 
                                        partition_states: dict):
        """
        Sync environmental changes from source to target partition in overlapping regions.
        This includes terrain obstacles, digging, and dumping - but NOT agent positions.
        """
        overlap_key = (source_partition, target_partition)
        if overlap_key not in self.overlap_regions:
            return
        ...
        # ALL maps that should be synchronized in overlapping regions
        # INCLUDING the base traversability (terrain obstacles)
        maps_to_sync = [
            'dumpability_mask', 
            'action_map', 
            'dig_map',
            'target_map',  # Target areas for digging/dumping
            'padding_mask'  # Boundary constraints
        ]
        
        updates_made = False
        
        # Sync all environmental maps
        for map_name in maps_to_sync:
            if hasattr(source_state.world, map_name) and hasattr(target_state.world, map_name):
                source_map = getattr(source_state.world, map_name).map
                target_map = getattr(target_state.world, map_name).map
                ...
                if not jnp.array_equal(source_overlap_data, target_overlap_data):
                    ...
                    updates_made = True
                    print(f"    Synced {map_name} changes to partition {target_partition}")
        
        # SPECIAL HANDLING FOR TRAVERSABILITY BASE (terrain obstacles only)
        # We need to sync the base terrain but preserve agent-specific modifications
        source_traversability = source_state.world.traversability_mask.map
        target_traversability = target_state.world.traversability_mask.map
        
        # Extract base terrain from source (everything that's not agent-related)
        # Terrain obstacles are typically positive values (1), free space is 0
        # Agent positions are -1, so we preserve those in target
        source_terrain_data = source_traversability[source_slice]
        target_terrain_data = target_traversability[target_slice]
        
        # Create a mask for where to update terrain
        # Only update cells that are terrain-related (not agent positions)
        terrain_update_mask = (target_terrain_data != -1)  # Don't overwrite agent positions
        
        # Apply terrain updates only where appropriate
        updated_terrain_data = jnp.where(
            terrain_update_mask,
            source_terrain_data,
            target_terrain_data
        )
        
        if not jnp.array_equal(updated_terrain_data, target_terrain_data):
            # Update the traversability in the overlap region
            updated_traversability = target_traversability.at[target_slice].set(updated_terrain_data)
            
            # Update the target state
            updated_world = self._update_world_map(target_state.world, 'traversability_mask', updated_traversability)
            updated_state = target_state._replace(world=updated_world)
            updated_timestep = partition_states[target_partition]['timestep']._replace(state=updated_state)
            partition_states[target_partition]['timestep'] = updated_timestep
            
            updates_made = True
            print(f"    Synced terrain obstacles to partition {target_partition}")
        
        if updates_made:
            print(f"  Environmental sync completed: {source_partition} -> {target_partition}")
    ...
    def _update_traversability_with_other_agents(self, target_partition_idx: int, partition_states: dict):
        """
        Update ONLY agent positions in traversability mask, preserving terrain obstacles.
        """
        if target_partition_idx not in partition_states:
            return
        
        target_partition_state = partition_states[target_partition_idx]
        if target_partition_state['status'] != 'active' or target_partition_state['timestep'] is None:
            return
        ...
        # PRESERVE terrain obstacles (positive values) and free space (0)
        # Only remove old agent positions (-1)
        base_traversability = jnp.where(current_traversability == -1, 0, current_traversability)
        
        # Add current agent's position back as -1
        current_agent_state = current_timestep.state.agent.agent_state
        current_pos = current_agent_state.pos_base
        
        # Extract current agent position
        if hasattr(current_pos, 'shape'):
            if len(current_pos.shape) == 1 and current_pos.shape[0] == 2:
                curr_pos_y = int(current_pos[0])
                curr_pos_x = int(current_pos[1])
            else:
                curr_pos_y = int(current_pos.flatten()[0])
                curr_pos_x = int(current_pos.flatten()[1])
        else:
            curr_pos_y = int(current_pos[0])
            curr_pos_x = int(current_pos[1])
        
        # Mark current agent's footprint as -1
        agent_height = self.small_agent_config['height'][0]
        agent_width = self.small_agent_config['width'][0]
        half_height = agent_height // 2
        half_width = agent_width // 2
        
        curr_y_start = max(0, curr_pos_y - half_height)
        curr_y_end = min(base_traversability.shape[0], curr_pos_y + half_height + 1)
        curr_x_start = max(0, curr_pos_x - half_width)
        curr_x_end = min(base_traversability.shape[1], curr_pos_x + half_width + 1)
        
        base_traversability = base_traversability.at[
            curr_y_start:curr_y_end, 
            curr_x_start:curr_x_end
        ].set(-1)
        
        # Add other agents as obstacles (value 1) ONLY in free space
        agents_added = 0
        for other_partition_idx, other_partition_state in partition_states.items():
            if (other_partition_idx == target_partition_idx or 
                other_partition_state['status'] != 'active' or 
                other_partition_state['timestep'] is None):
                continue
            
            other_partition = self.partitions[other_partition_idx]
            other_region_coords = other_partition['region_coords']
            
            # Check if the other agent's region overlaps with target region
            if not self._partitions_overlap(target_partition_idx, other_partition_idx):
                continue
            ...
            # Extract position values
            if hasattr(other_pos, 'shape'):
                if len(other_pos.shape) == 1 and other_pos.shape[0] == 2:
                    other_pos_y = float(other_pos[0])
                    other_pos_x = float(other_pos[1])
                else:
                    other_pos_y = float(other_pos.flatten()[0])
                    other_pos_x = float(other_pos.flatten()[1])
            else:
                other_pos_y = float(other_pos[0])
                other_pos_x = float(other_pos[1])
            
            # Convert to global coordinates
            global_other_y = other_pos_y + other_region_coords[0]
            global_other_x = other_pos_x + other_region_coords[1]
            
            # Check if this global position falls within the target partition's region
            if (target_y_start <= global_other_y <= target_y_end and 
                target_x_start <= global_other_x <= target_x_end):
                
                # Convert global position to local coordinates in target partition
                local_other_y = int(global_other_y - target_y_start)
                local_other_x = int(global_other_x - target_x_start)
                
                # Ensure the position is within bounds
                if (0 <= local_other_y < base_traversability.shape[0] and 
                    0 <= local_other_x < base_traversability.shape[1]):
                    
                    # Calculate the other agent's footprint
                    other_y_start = max(0, local_other_y - half_height)
                    other_y_end = min(base_traversability.shape[0], local_other_y + half_height + 1)
                    other_x_start = max(0, local_other_x - half_width)
                    other_x_end = min(base_traversability.shape[1], local_other_x + half_width + 1)
                    
                    # Only mark as obstacle where there's currently free space (0) or agent space (-1)
                    # Don't overwrite existing terrain obstacles
                    for y in range(other_y_start, other_y_end):
                        for x in range(other_x_start, other_x_end):
                            if base_traversability[y, x] <= 0:  # Free space or agent space
                                base_traversability = base_traversability.at[y, x].set(1)
                    
                    agents_added += 1
                    print(f"  Added agent {other_partition_idx} as obstacle at local ({local_other_y}, {local_other_x}) "
                        f"in partition {target_partition_idx}")
        
        print(f"  Partition {target_partition_idx}: Added {agents_added} other agents as obstacles")
        
        # Update the traversability mask in the target partition's state
        updated_world = self._update_world_map(
            current_timestep.state.world, 
            'traversability_mask', 
            base_traversability
        ...
        partition_states[target_partition_idx]['timestep'] = updated_timestep
    # def render_partition_debug_views(self, partition_states):
    #     """
    #     Render debug views showing what each partition sees, including other agents as obstacles.
    #     This creates a separate window or saves images showing the traversability mask for each partition.
    #     """
    #     import pygame as pg
    #     import numpy as np
    ...
    #     debug_screen = pg.display.set_mode((debug_window_width, debug_window_height))
    #     pg.display.set_caption("Partition Debug Views")
        
    #     # Colors for visualization
    #     colors = {
    #         -1: (255, 0, 255),    # Agent (magenta)
    #         0: (255, 255, 255),   # Free space (white)
    #         1: (0, 0, 0),         # Obstacle/other agent (black)
    #         2: (128, 128, 128)    # Gray (if used)
    #     }
    ...
    #     # Calculate grid layout for partitions
    #     num_partitions = len(active_partitions)
    #     cols = int(np.ceil(np.sqrt(num_partitions)))
    #     rows = int(np.ceil(num_partitions / cols))
        
    #     partition_width = debug_window_width // cols
    #     partition_height = debug_window_height // rows
        
    #     debug_screen.fill((50, 50, 50))  # Dark gray background
        
    #     for i, partition_idx in enumerate(active_partitions):
    #         partition_state = partition_states[partition_idx]
            
    #         # Get the current traversability mask
    #         traversability_mask = partition_state['timestep'].state.world.traversability_mask.map
            
    #         # Calculate position in debug window
    #         col = i % cols
    #         row = i // cols
    ...
    #         self._render_single_partition_debug(
    #             debug_screen, traversability_mask, partition_idx, 
    #             x_offset, y_offset, partition_width, partition_height, colors
    #         )
    ...
    # def _render_single_partition_debug(self, screen, traversability_mask, partition_idx, 
    #                                 x_offset, y_offset, width, height, colors):
    #     """
    #     Render a single partition's traversability mask for debugging.
    #     """
    #     import pygame as pg
        
    #     # Get map dimensions
    #     map_height, map_width = traversability_mask.shape
        
    #     # Calculate tile size for this debug view
    #     tile_width = width // map_width
    #     tile_height = height // map_height
    #     tile_size = min(tile_width, tile_height, 8)  # Cap at 8 pixels per tile
        
    #     # Draw the traversability mask
    #     for y in range(map_height):
    #         for x in range(map_width):
    #             value = int(traversability_mask[y, x])
    #             color = colors.get(value, (128, 0, 128))  # Purple for unknown values
    ...
    #     # Draw border around this partition view
    #     border_rect = pg.Rect(x_offset, y_offset, map_width * tile_size, map_height * tile_size)
    #     pg.draw.rect(screen, (255, 255, 255), border_rect, 2)
        
    #     # Add partition label
    #     font = pg.font.Font(None, 24)
    #     text = font.render(f"Partition {partition_idx}", True, (255, 255, 255))
    #     screen.blit(text, (x_offset + 5, y_offset + 5))

    # def save_partition_debug_images(self, partition_states, step_number):
    #     """
    #     Save debug images showing what each partition sees.
    ...
    #     debug_dir = "debug_partition_views"
    ...
    #             'white',    # 0: free space
    #             'black',    # 1: obstacle/other agent
    #             'magenta'   # -1: current agent (if present)
    #         ])
            
    #         # Plot the mask
    #         plt.imshow(traversability_mask, cmap=cmap, vmin=-1, vmax=1)
    #         plt.title(f'Partition {partition_idx} View - Step {step_number}')
    #         plt.colorbar(label='Traversability (-1: agent, 0: free, 1: obstacle)')
            
    #         # Add grid
    #         plt.grid(True, alpha=0.3)
            
    #         # Save the image
    #         filename = f"{debug_dir}/partition_{partition_idx}_step_{step_number:04d}.png"
    #         plt.savefig(filename, dpi=150, bbox_inches='tight')
    #         plt.close()
            
    #         print(f"Saved debug view: {filename}")

    # def print_partition_debug_info(self, partition_states):
    #     """
    #     Print text-based debug information about what each partition sees.
    #     """
    ...
    #         # Get agent position
    #         agent_state = partition_state['timestep'].state.agent.agent_state
    ...
    #         agent_cells = np.sum(traversability_mask == -1)
    #         free_cells = np.sum(traversability_mask == 0)
    #         obstacle_cells = np.sum(traversability_mask == 1)
            
    #         print(f"Partition {partition_idx}:")
    #         print(f"  Agent position: ({agent_pos[0]:.1f}, {agent_pos[1]:.1f})")
    #         print(f"  Agent cells (-1): {agent_cells}")
    #         print(f"  Free cells (0): {free_cells}")
    #         print(f"  Obstacle cells (1): {obstacle_cells}")
            
    #         # Find other agents (obstacles that might be other agents)
    #         # Look for clusters of obstacles that might represent other agents
    #         obstacle_positions = np.where(traversability_mask == 1)
    #         if len(obstacle_positions[0]) > 0:
    #             print(f"  Obstacle positions (potential other agents):")
    #             for i in range(min(5, len(obstacle_positions[0]))):  # Show first 5
    #                 y, x = obstacle_positions[0][i], obstacle_positions[1][i]
    #                 print(f"    ({x}, {y})")
    #         print()

    # # Add this to your main game loop for debugging
    # def debug_step_with_partition_views(self, partition_states, step_number, debug_frequency=10):
    #     """
    #     Enhanced stepping with debug visualization.
    #     Call this instead of regular stepping when you want to debug.
    #     """
    #     # Print debug info every few steps
    #     if step_number % debug_frequency == 0:
    #         self.print_partition_debug_info(partition_states)
            
    #         # Uncomment one of these for visual debugging:
    #         # self.render_partition_debug_views(partition_states)  # Real-time window
    #         # self.save_partition_debug_images(partition_states, step_number)  # Save images
        
    #     return True
    
    def render_local_partition_views(self, partition_states):
        """
        Render the local partition views instead of the global view.
        This shows what each agent actually sees, including other agents as obstacles.
        """
        active_partitions = [idx for idx, state in partition_states.items() 
                            if state['status'] == 'active']
        ...
        # Use the first active partition for rendering
        partition_idx = active_partitions[0]
        partition_state = partition_states[partition_idx]
        current_timestep = partition_state['timestep']
        
        # Extract the observation and info for rendering
        obs = current_timestep.observation
        info = current_timestep.info
        
        # Add debug info to show which partition we're viewing
        info['debug_partition_id'] = partition_idx
        info['debug_active_partitions'] = active_partitions
        
        # Render using the small environment's renderer
        self.small_env.terra_env.render_obs_pygame(obs, info)
        
        print(f"Rendering partition {partition_idx} view (agent sees other agents as obstacles)")
    ...
    def render_simple_debug_view(self, partition_states):
        """
        Very simple debug rendering - just print what each partition sees.
        """
        print(f"\n=== PARTITION VIEWS DEBUG ===")
        ...
        for partition_idx, partition_state in partition_states.items():
            if partition_state['status'] == 'active':
                ...
                # Count obstacles (other agents and terrain)
                obstacles = np.sum(traversability == 1)
                free_cells = np.sum(traversability == 0)
                
                print(f"Partition {partition_idx}:")
                print(f"  Agent at: ({agent_pos[0]:.1f}, {agent_pos[1]:.1f})")
                print(f"  Sees {obstacles} obstacles (other agents + terrain)")
                print(f"  Has {free_cells} free cells")
        
        print("=" * 40)
    def debug_partition_overlap_details(self, partition_states):
        """
        Detailed debugging to understand overlap issues.
        """
        print(f"\n=== DETAILED OVERLAP DEBUG ===")
        
        # Print partition definitions
        print("Partition definitions:")
        for i, partition in enumerate(self.partitions):
            print(f"  Partition {i}: {partition['region_coords']}")
        
        # Print overlap map
        print("\nOverlap relationships:")
        for i, overlaps in self.overlap_map.items():
            print(f"  Partition {i} overlaps with: {list(overlaps)}")
        
        # Detailed overlap region analysis
        print("\nOverlap region details:")
        for (i, j), overlap_info in self.overlap_regions.items():
            if overlap_info is None:
                continue
            print(f"  Partitions {i}<->{j}:")
            print(f"    Global slice: {overlap_info['global_slice']}")
            print(f"    Partition {i} slice: {overlap_info['partition_i_slice']}")
            print(f"    Partition {j} slice: {overlap_info['partition_j_slice']}")
            print(f"    Overlap bounds: {overlap_info['overlap_bounds']}")
        
        # Check actual data in overlap regions
        print("\nActual data in overlap regions:")
        for i in range(len(self.partitions)):
            for j in range(i + 1, len(self.partitions)):
                if not self._partitions_overlap(i, j):
                    continue
                ...
                overlap_info = self.overlap_regions.get((i, j))
                if overlap_info is None:
                    continue
                ...
                print(f"  Overlap {i}<->{j}:")
                print(f"    Partition {i} overlap shape: {overlap_data_i.shape}")
                print(f"    Partition {j} overlap shape: {overlap_data_j.shape}")
                print(f"    Data matches: {jnp.array_equal(overlap_data_i, overlap_data_j)}")
                
                if not jnp.array_equal(overlap_data_i, overlap_data_j):
                    print(f"    Differences found!")
                    # Show some sample values
                    print(f"    Sample from partition {i}: {overlap_data_i.flatten()[:10]}")
                    print(f"    Sample from partition {j}: {overlap_data_j.flatten()[:10]}")

    def force_synchronize_all_overlaps(self, partition_states):
        """
        Force synchronization of ALL overlapping regions immediately.
        This should be called after any environment changes.
        """
        print(f"\n=== FORCE SYNCHRONIZING ALL OVERLAPS ===")
        
        # Get the global "truth" by updating global maps first
        for partition_idx, partition_state in partition_states.items():
            if partition_state['status'] == 'active':
                self._update_global_maps_from_partition(partition_idx, partition_state['timestep'])
        
        # Now propagate the global truth back to all overlapping regions
        for partition_idx, partition_state in partition_states.items():
            if partition_state['status'] != 'active':
                continue
            ...
            print(f"Force syncing partition {partition_idx}...")
            
            # Extract the "truth" from global maps for this partition's region
            global_slice = (slice(y_start, y_end + 1), slice(x_start, x_end + 1))
            
            # Get current state
            current_timestep = partition_state['timestep']
            current_state = current_timestep.state
            
            # Maps to sync from global to local
            maps_to_sync = [
                'target_map',
                'action_map', 
                'dumpability_mask',
            ...
            # Update each map from global truth
            updated_world = current_state.world
            
            for map_name in maps_to_sync:
                if map_name in self.global_maps:
                    global_map = self.global_maps[map_name]
                    local_truth = global_map[global_slice]
                    
                    # Update the local map
                    updated_world = self._update_world_map(updated_world, map_name, local_truth)
                    print(f"  Updated {map_name} from global truth")
            
            # Special handling for traversability - preserve agent positions
            global_traversability = self.global_maps.get('traversability_mask')
            if global_traversability is not None:
                local_traversability_truth = global_traversability[global_slice]
                current_traversability = current_state.world.traversability_mask.map
                
                # Preserve current agent positions (-1) but update everything else
                current_agent_mask = (current_traversability == -1)
                
                # Use global truth where there's no current agent
                updated_traversability = jnp.where(
                    current_agent_mask,
                    current_traversability,  # Keep agent positions
                    local_traversability_truth  # Use global truth elsewhere
                )
                
                updated_world = self._update_world_map(updated_world, 'traversability_mask', updated_traversability)
                print(f"  Updated traversability_mask from global truth (preserving agent)")
    ...
    # def step_with_overlap_updates_fixed(self, partition_idx: int, action, partition_states: dict):
    #     """
    #     COMPLETELY REVISED VERSION: Simple and robust synchronization.
    #     """
    ...
    #     print(f"Partition {partition_idx} stepped successfully")
        
    #     return new_timestep
    # def step_with_overlap_updates(self, partition_idx: int, action, partition_states: dict):
    #     """
    #     Your existing step function with minimal changes - just removed complex sync.
    #     Synchronization happens separately after all partitions step.
    #     """
    #     # Step the environment normally first
    #     partition_state = partition_states[partition_idx]
    #     current_state = partition_state['timestep'].state
    ...
    #     # Step the environment - THIS GIVES YOU THE REWARD
    #     new_timestep = self.small_env.step(
    #         state=current_state,
    #         action=action,
    #         target_map=current_target_map,
    ...
    #     # Update partition state - REWARD IS PRESERVED HERE
    #     partition_states[partition_idx]['timestep'] = new_timestep
        
    #     # Update global maps with changes from this partition
    #     self._update_global_maps_from_partition(partition_idx, new_timestep)
        
    #     # Return the timestep with the reward intact
    #     return new_timestep

    def step_with_overlap_updates(self, partition_idx: int, action, partition_states: dict):
        """
        MINIMAL VERSION: Just step the environment, no sync during step.
        All sync happens after all partitions step.
        """
        # Step the environment normally
        partition_state = partition_states[partition_idx]
        current_state = partition_state['timestep'].state
        ...
        # Step the environment - PRESERVES REWARD
        new_timestep = self.small_env.step(
            state=current_state,
            action=action,
            target_map=current_target_map,
        ...
        # Just return the timestep, no sync here
        return new_timestep
    def safe_sync_after_all_steps(self, partition_states):
        """
        Safe synchronization that only fixes terrain and adds other agents.
        Never touches individual agent positions.
        """
        print(f"\n=== SAFE SYNC AFTER ALL STEPS ===")
        
        # Step 1: Sync terrain obstacles between overlapping partitions
        self.sync_only_terrain_obstacles(partition_states)
        
        # Step 2: Add other agents as obstacles (without affecting terrain or current agents)
        self.add_other_agents_as_obstacles_safe(partition_states)
        

    # def synchronize_all_partitions_after_step(self, partition_states):
    #     """
    #     Call this ONCE after ALL partitions have stepped.
    #     This ensures complete synchronization.
    #     """
    #     print(f"\n=== SYNCHRONIZING ALL PARTITIONS ===")
        
    #     # Step 1: Force sync all environmental data
    #     self.force_synchronize_all_overlaps(partition_states)
        
    #     # Step 2: Update agent positions for all partitions
    #     for partition_idx in partition_states.keys():
    #         if partition_states[partition_idx]['status'] == 'active':
    #             self._update_traversability_with_other_agents_simple(partition_idx, partition_states)
        
    #     print("All partitions synchronized.")

    # def _update_traversability_with_other_agents_simple(self, target_partition_idx: int, partition_states: dict):
    #     """
    #     Simplified version that just adds other agents as obstacles.
    #     """
    #     if target_partition_idx not in partition_states:
    #         return
    ...
    #     # Remove any existing other agent markers (but keep terrain and current agent)
    #     # Assume other agents were marked as value 2 to distinguish from terrain (1)
    #     # Reset any cells that might have been other agents back to free space
    #     # traversability = jnp.where(traversability == 2, 0, traversability)
    ...
    #                 # Mark as obstacle (value 2 to distinguish from terrain)
    #                 # Use a small footprint around the agent
    #                 for dy in range(-2, 3):
    #                     for dx in range(-2, 3):
    #                         y, x = local_other_y + dy, local_other_x + dx
    #                         if (0 <= y < traversability.shape[0] and 
    #                             0 <= x < traversability.shape[1]):
    #                             # Only mark as other agent if it's currently free space
    #                             if traversability[y, x] == 0:
    #                                 traversability = traversability.at[y, x].set(2)
                    
    #                 agents_added += 1
    #                 print(f"  Added other agent {other_partition_idx} at ({local_other_y}, {local_other_x})")
    ...
    #         print(f"  Partition {target_partition_idx}: Added {agents_added} other agents")

    # # Updated main loop integration
    # def step_all_partitions_with_sync(self, active_partitions, partition_states, wrapped_actions):
    #     """
    #     Step all partitions and ensure proper synchronization.
    #     Call this instead of stepping partitions individually.
    #     """
    #     print(f"\n=== STEPPING ALL {len(active_partitions)} PARTITIONS ===")
        
    #     # Step each partition individually first
    #     for i, partition_idx in enumerate(active_partitions):
    #         wrapped_action = wrapped_actions[i]
    #         print(f"Stepping partition {partition_idx}...")
            
    #         new_timestep = self.step_with_overlap_updates_fixed(
    ...
    #     self.debug_partition_overlap_details(partition_states)
        
    #     # Now synchronize everything
    #     self.synchronize_all_partitions_after_step(partition_states)
        
    #     # Debug after sync
    #     print("\nAFTER synchronization:")
    #     self.debug_partition_overlap_details(partition_states)
    def simple_sync_overlaps_after_all_steps(self, partition_states):
        """
        Simple synchronization function to call AFTER all partitions have stepped.
        This preserves your existing reward tracking and just fixes the sync issue.
        """
        print(f"Synchronizing overlaps between {len(partition_states)} partitions...")
        
        # Step 1: Update global maps from all partitions
        for partition_idx, partition_state in partition_states.items():
            if partition_state['status'] == 'active' and partition_state['timestep'] is not None:
                self._update_global_maps_from_partition(partition_idx, partition_state['timestep'])
        
        # Step 2: Force sync overlapping regions between partitions
        for i in range(len(self.partitions)):
            for j in range(i + 1, len(self.partitions)):
                if not self._partitions_overlap(i, j):
                    continue
                ...
                # Force sync between these two partitions
                self._force_sync_two_partitions(i, j, partition_states)
        
        # Step 3: Update agent positions for all partitions
        for partition_idx in partition_states.keys():
            if partition_states[partition_idx]['status'] == 'active':
                self._simple_update_agent_obstacles(partition_idx, partition_states)
    def _force_sync_two_partitions(self, partition_i: int, partition_j: int, partition_states: dict):
        """
        Force synchronization between two overlapping partitions.
        """
        overlap_key = (partition_i, partition_j)
        if overlap_key not in self.overlap_regions:
            return
        ...
        print(f"  Force syncing partitions {partition_i} <-> {partition_j}")
        
        # Get the overlap region coordinates
        slice_i = overlap_info['partition_i_slice']
        slice_j = overlap_info['partition_j_slice']
        global_bounds = overlap_info['overlap_bounds']
        
        # Use global map as the source of truth
        global_y_start, global_x_start, global_y_end, global_x_end = global_bounds
        global_slice = (slice(global_y_start, global_y_end + 1), slice(global_x_start, global_x_end + 1))
        
        # Maps to synchronize
        maps_to_sync = ['traversability_mask', 'target_map', 'action_map', 'dumpability_mask', 'dig_map', 'padding_mask']
        
        for map_name in maps_to_sync:
            if map_name not in self.global_maps:
                continue
                
            # Get the truth from global map
            global_truth = self.global_maps[map_name][global_slice]
            
            # Update both partitions with this truth
            for partition_idx, local_slice in [(partition_i, slice_i), (partition_j, slice_j)]:
                current_timestep = partition_states[partition_idx]['timestep']
                current_map = getattr(current_timestep.state.world, map_name).map
                
                # Update the overlap region
                updated_map = current_map.at[local_slice].set(global_truth)
                
                # Update the world state
                updated_world = self._update_world_map(current_timestep.state.world, map_name, updated_map)
                updated_state = current_timestep.state._replace(world=updated_world)
                updated_timestep = current_timestep._replace(state=updated_state)
                
                # Update partition state
                partition_states[partition_idx]['timestep'] = updated_timestep
    def sync_only_terrain_obstacles(self, partition_states):
        """
        MINIMAL SAFE SYNC: Only synchronize terrain obstacles, never touch agent positions.
        This preserves agent positions and only fixes terrain inconsistencies.
        """
        print(f"Syncing terrain obstacles between overlapping partitions...")
        ...
        for i in range(len(self.partitions)):
            for j in range(i + 1, len(self.partitions)):
                ...
                self._sync_terrain_between_two_partitions(i, j, partition_states)

    def _sync_terrain_between_two_partitions(self, partition_i: int, partition_j: int, partition_states: dict):
        """
        Sync ONLY terrain obstacles between two partitions, preserving agent positions.
        """
        overlap_key = (partition_i, partition_j)
        if overlap_key not in self.overlap_regions:
            return
        ...
        print(f"  Syncing terrain between partitions {partition_i} <-> {partition_j}")
        
        # Get the overlap slices
        slice_i = overlap_info['partition_i_slice']
        slice_j = overlap_info['partition_j_slice']
        
        # Get current states
        state_i = partition_states[partition_i]['timestep'].state
        state_j = partition_states[partition_j]['timestep'].state
        
        # Get traversability masks
        trav_i = state_i.world.traversability_mask.map
        trav_j = state_j.world.traversability_mask.map
        ...
        # Create terrain masks (only terrain obstacles, value > 0 but not agent -1)
        terrain_mask_i = (overlap_i > 0)  # Terrain obstacles
        terrain_mask_j = (overlap_j > 0)  # Terrain obstacles
        
        # If terrain doesn't match, use the union (any obstacle in either partition)
        if not jnp.array_equal(terrain_mask_i, terrain_mask_j):
            print(f"    Found terrain differences, syncing...")
            
            # Create unified terrain (union of obstacles from both partitions)
            unified_terrain_mask = terrain_mask_i | terrain_mask_j
            
            # Update partition i's overlap region (preserve agents)
            new_overlap_i = overlap_i.copy()
            # Set terrain obstacles where unified mask is True and current cell is not an agent
            new_overlap_i = jnp.where(
                unified_terrain_mask & (overlap_i != -1),  # Where there should be terrain AND no agent
                1,  # Set as terrain obstacle
                jnp.where(
                    ~unified_terrain_mask & (overlap_i > 0),  # Where there shouldn't be terrain BUT there is
                    0,  # Clear terrain obstacle (but keep agents)
                    overlap_i  # Keep everything else as is
                )
            )
            
            # Update partition j's overlap region (preserve agents)
            new_overlap_j = overlap_j.copy()
            new_overlap_j = jnp.where(
                unified_terrain_mask & (overlap_j != -1),  # Where there should be terrain AND no agent
                1,  # Set as terrain obstacle
                jnp.where(
                    ~unified_terrain_mask & (overlap_j > 0),  # Where there shouldn't be terrain BUT there is
                    0,  # Clear terrain obstacle (but keep agents)
                    overlap_j  # Keep everything else as is
                )
            )
            ...
            updated_world_i = self._update_world_map(state_i.world, 'traversability_mask', updated_trav_i)
            updated_state_i = state_i._replace(world=updated_world_i)
            updated_timestep_i = partition_states[partition_i]['timestep']._replace(state=updated_state_i)
            partition_states[partition_i]['timestep'] = updated_timestep_i
            
            # Update partition j
            updated_trav_j = trav_j.at[slice_j].set(new_overlap_j)
            updated_world_j = self._update_world_map(state_j.world, 'traversability_mask', updated_trav_j)
            updated_state_j = state_j._replace(world=updated_world_j)
            updated_timestep_j = partition_states[partition_j]['timestep']._replace(state=updated_timestep_j)
            partition_states[partition_j]['timestep'] = updated_timestep_j
            
            print(f"    Terrain sync completed for partitions {partition_i} <-> {partition_j}")

    def add_other_agents_as_obstacles_safe(self, partition_states):
        """
        Safely add other agents as obstacles without affecting terrain.
        """
        print(f"Adding other agents as obstacles...")
        ...
        for target_partition_idx, target_partition_state in partition_states.items():
            ...
            for other_partition_idx, other_partition_state in partition_states.items():
                ...
                if (target_y_start <= global_other_y <= target_y_end and 
                    ...
                    if (0 <= local_other_y < traversability.shape[0] and 
                        ...
                        # Add a small obstacle around the other agent
                        # Only mark FREE SPACE (value 0) as obstacles
                        for dy in range(-2, 3):  # 5x5 area around agent
                            for dx in range(-2, 3):
                                y, x = local_other_y + dy, local_other_x + dx
                                if (0 <= y < traversability.shape[0] and 
                                    0 <= x < traversability.shape[1]):
                                    # Only mark as obstacle if it's currently free space
                                    if traversability[y, x] == 0:
                                        traversability = traversability.at[y, x].set(1)
                        
                        agents_added += 1
                        print(f"  Added agent {other_partition_idx} as obstacle in partition {target_partition_idx}")
            ...
            if agents_added > 0:
                ...
                partition_states[target_partition_idx]['timestep'] = updated_timestep
    def _simple_update_agent_obstacles(self, target_partition_idx: int, partition_states: dict):
            """
            Simple agent obstacle update - just mark other agents as obstacles.
            """
            target_partition_state = partition_states[target_partition_idx]
            if target_partition_state['status'] != 'active':
                return
            ...
            # Find other agents and mark them as obstacles
            for other_partition_idx, other_partition_state in partition_states.items():
                if (other_partition_idx == target_partition_idx or 
                    other_partition_state['status'] != 'active'):
                    continue
                ...
                # Check if this agent is in the target partition's region
                if (target_y_start <= global_other_y <= target_y_end and 
                    target_x_start <= global_other_x <= target_x_end):
                    ...
                    if (0 <= local_other_y < traversability.shape[0] and 
                        ...
                        # Mark as obstacle (use value 1, same as terrain obstacles)
                        # Create a small footprint around the agent
                        agent_size = 3  # 3x3 footprint
                        half_size = agent_size // 2
            ...
            # Update the traversability mask
            updated_world = self._update_world_map(
                current_timestep.state.world, 
                'traversability_mask', 
                traversability
    ...
    def debug_partition_maps(self, partition_states):
        """
        Print detailed information about what each partition contains.
        """
        print(f"\n=== PARTITION MAP DEBUG ===")
        ...
        for partition_idx, partition_state in partition_states.items():
            ...
            agent_cells = jnp.sum(traversability == -1)
            free_cells = jnp.sum(traversability == 0)
            obstacle_cells = jnp.sum(traversability == 1)
            other_values = jnp.sum((traversability != -1) & (traversability != 0) & (traversability != 1))
            
            # Action map stats
            dug_cells = jnp.sum(action_map == -1)
            dumped_cells = jnp.sum(action_map > 0)
            
            # Target map stats  
            dig_targets = jnp.sum(target_map == -1)
            dump_targets = jnp.sum(target_map == 1)
            
            print(f"  Traversability: Agent(-1)={agent_cells}, Free(0)={free_cells}, Obstacles(1)={obstacle_cells}, Other={other_values}")
            print(f"  Action: Dug(-1)={dug_cells}, Dumped(>0)={dumped_cells}")
            print(f"  Target: Dig(-1)={dig_targets}, Dump(1)={dump_targets}")
            
            # Agent position
            agent_state = timestep.state.agent.agent_state
            agent_pos = agent_state.pos_base
            print(f"  Agent position: ({agent_pos[0]:.1f}, {agent_pos[1]:.1f})")

    def debug_overlap_regions(self, partition_states):
        """
        Debug what's actually in the overlap regions.
        """
        print(f"\n=== OVERLAP REGION DEBUG ===")
        ...
        for i in range(len(self.partitions)):
            for j in range(i + 1, len(self.partitions)):
                ...
                print(f"\nOverlap between partitions {i} and {j}:")
                
                # Get overlap info
                overlap_info = self.overlap_regions.get((i, j))
                if overlap_info is None:
                    print("  No overlap info found!")
                    continue
                    
                slice_i = overlap_info['partition_i_slice']
                slice_j = overlap_info['partition_j_slice']
                
                # Get the actual data in overlap regions
                trav_i = partition_states[i]['timestep'].state.world.traversability_mask.map
                trav_j = partition_states[j]['timestep'].state.world.traversability_mask.map
                ...
                # Count cell types in overlap
                for partition_idx, overlap_data in [(i, overlap_data_i), (j, overlap_data_j)]:
                    agent_cells = jnp.sum(overlap_data == -1)
                    free_cells = jnp.sum(overlap_data == 0)
                    obstacle_cells = jnp.sum(overlap_data == 1)
                    print(f"  Partition {partition_idx} overlap: Agent(-1)={agent_cells}, Free(0)={free_cells}, Obstacles(1)={obstacle_cells}")
                
                # Check if they match
                matches = jnp.array_equal(overlap_data_i, overlap_data_j)
                print(f"  Data matches: {matches}")
                
                if not matches:
                    diff_count = jnp.sum(overlap_data_i != overlap_data_j)
                    print(f"  {diff_count} cells differ out of {overlap_data_i.size}")
                    
                    # Show some specific differences
                    diff_positions = jnp.where(overlap_data_i != overlap_data_j)
                    if len(diff_positions[0]) > 0:
                        for idx in range(min(5, len(diff_positions[0]))):  # Show first 5 differences
                            y, x = diff_positions[0][idx], diff_positions[1][idx]
                            val_i = overlap_data_i[y, x]
                            val_j = overlap_data_j[y, x]
                            print(f"    Diff at ({y},{x}): partition {i}={val_i}, partition {j}={val_j}")

    # EXTREMELY SIMPLE sync - just copy from one partition to another
    def simple_copy_sync(self, partition_states):
        """
        Ultra-simple sync: just copy traversability from first partition to overlapping regions of others.
        This is for testing to see if the sync mechanism works at all.
        """
        print(f"\n=== SIMPLE COPY SYNC TEST ===")
        
        active_partitions = [idx for idx, state in partition_states.items() if state['status'] == 'active']
        if len(active_partitions) < 2:
            return
        
        # Use first partition as the "source of truth"
        source_partition_idx = active_partitions[0]
        source_state = partition_states[source_partition_idx]['timestep'].state
        source_traversability = source_state.world.traversability_mask.map
        
        print(f"Using partition {source_partition_idx} as source of truth")
        
        # Copy to all overlapping partitions
        for target_partition_idx in active_partitions[1:]:
            if not self._partitions_overlap(source_partition_idx, target_partition_idx):
                continue
                
            print(f"  Copying from {source_partition_idx} to {target_partition_idx}")
            
            overlap_key = (source_partition_idx, target_partition_idx)
            overlap_info = self.overlap_regions.get(overlap_key)
            
            if overlap_info is None:
                print(f"    No overlap info found!")
                continue
                
            source_slice = overlap_info['partition_i_slice']
            target_slice = overlap_info['partition_j_slice']
            
            # Get source overlap data
            source_overlap_data = source_traversability[source_slice]
            
            # Get target state
            target_state = partition_states[target_partition_idx]['timestep'].state
            target_traversability = target_state.world.traversability_mask.map
            
            # Copy the overlap data (THIS IS A TEST - normally we'd be more careful)
            updated_target_traversability = target_traversability.at[target_slice].set(source_overlap_data)
            
            # Update the target partition
            updated_world = self._update_world_map(
                target_state.world, 
                'traversability_mask', 
                updated_target_traversability
            )
            updated_state = target_state._replace(world=updated_world)
            ...
            print(f"    Copied {source_overlap_data.shape} cells")

    # Add these to your main loop for debugging
    def run_diagnosis(self, partition_states):
        """
        Run full diagnosis to understand the synchronization problem.
        """
        print(f"\n" + "="*50)
        print(f"RUNNING FULL DIAGNOSIS")
        print(f"="*50)
        
        self.debug_partition_maps(partition_states)
        self.debug_overlap_regions(partition_states)
        
        print(f"\nTesting simple copy sync...")
        self.simple_copy_sync(partition_states)
        
        print(f"\nAfter simple copy sync:")
        self.debug_overlap_regions(partition_states)
        
        print(f"="*50)

    # Minimal step function - no sync at all
    def step_with_no_sync(self, partition_idx: int, action, partition_states: dict):
        """
        Step with absolutely no synchronization - for testing.
        """
        partition_state = partition_states[partition_idx]
        current_state = partition_state['timestep'].state
        current_env_cfg = partition_state['timestep'].env_cfg
...
# Add this at the top of your file with other configuration constants
DEBUG_PARTITION_VIEWS = True  # Set to True to enable debug visualization
DEBUG_FREQUENCY = 5  # Debug every N steps
DEBUG_MODE = "save+images"  # Options: "print", "window", "save_images"
...
def run_experiment_with_disjoint_environments(
    ...
    while playing and step < num_timesteps:
        ...
        # DEBUG: Visualize what each partition sees
        # if DEBUG_PARTITION_VIEWS and step % DEBUG_FREQUENCY == 0:
        #     print(f"\n=== DEBUG STEP {step} ===")
            
        #     if DEBUG_MODE == "print":
        #         env_manager.print_partition_debug_info(partition_states)
            
        #     elif DEBUG_MODE == "window":
        #         # Show real-time debug window
        #         continue_debug = env_manager.render_partition_debug_views(partition_states)
        #         if not continue_debug:
        #             break  # User closed debug window
            
        #     elif DEBUG_MODE == "save_images":
        #         # Save debug images to disk
        #         env_manager.save_partition_debug_images(partition_states, step)
        ...
        # for target_partition_idx in active_partitions:
        #     env_manager._update_traversability_with_other_agents(target_partition_idx, partition_states)
        # env_manager.simple_sync_overlaps_after_all_steps(partition_states)
        ...
        #env_manager.render_local_partition_views(partition_states)
        if GRID_RENDERING:
