...
def run_experiment(llm_model_name, llm_model_key, num_timesteps, n_envs_x, n_envs_y, seed, progressive_gif, run):
    ...
    env_cfgs = jax.tree_map(
        lambda x: x[0][None, ...].repeat(4, 0), env_cfgs
    ) 
    #print(f"env_cfgs: {env_cfgs}")
    ...
    rng_reset = jax.random.split(_rng, 4)
    ...
    combined_obs = combine_maps_into_grid(obs)
    print(f"Combined observation: {combined_obs}")
    info = timestep.info
    print(f"Info: {info}")
    combined_info = extract_first_row(info)
    print(f"Combined info: {combined_info}")

    print(combined_obs["action_map"].shape)

    config.num_test_rollouts = 1

    env_cfgs_big = log["env_config"]
    print(f"env_cfgs_big: {env_cfgs_big}")

    env_cfgs_big = jax.tree_map(
        lambda x: x[0][None, ...].repeat(1, 0), env_cfgs_big
    ) 
    print(f"env_cfgs_big: {env_cfgs_big}")

    env_big = TerraEnvBatch(
        rendering=True,
        n_envs_x_rendering=n_envs_x,
        n_envs_y_rendering=n_envs_y,
        display=True,
    ...
    rng_big = jax.random.PRNGKey(seed)
    rng_big, _rng_big = jax.random.split(rng_big)
    rng_reset_big = jax.random.split(_rng_big, 1)
    timestep_big = env.reset(env_cfgs_big, rng_reset_big)
    print("timestep reset")
    print(combined_obs["action_map"].shape)

    # array_action = np.array(combined_obs["action_map"][0])
    # array_target = np.array(combined_obs["target_map"][0])

    # print(f"Array action shape: {array_action.shape}")
    # print(f"Array target shape: {array_target.shape}")
    # #print(f"Array action: {array_action}")
    # #flattened_array = array.flatten()
    # #print(f"Flattened array shape: {flattened_array.shape}")

    # im_action=Image.fromarray(array_action)
    # im_target=Image.fromarray(array_target)
    # im_action.save("action.png")
    # im_target.save("target.png")

    
    # screen = pg.display.get_surface()
    # frames = []
    
    def repeat_action_2(action, n_times=1):
        return action_type.new(action.action[None].repeat(n_times, 0))
    
    # Trigger the JIT compilation
    timestep_big = env_big.step(timestep_big, repeat_action_2(action_type.do_nothing()), rng_reset_big)
    env_big.terra_env.render_obs_pygame(combined_obs, combined_info)
    ...
    def repeat_action(action, n_times=4):
        return action_type.new(action.action[None].repeat(n_times, 0))
