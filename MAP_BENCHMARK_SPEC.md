...
| `R-20260727-36` | Accepted optimization gate | Sweep only exact service batch sizes `4/8/16` on the same pinned state and first 16 accepted rows, with five equal-work repeats, exact concatenated-output parity, honest padding, cumulative memory, and no complete-validator call or admission result. Select away from 4 only when the eligible winner's projected 256-scenario p95 is strictly below batch 4's p50, breaking ties toward the smaller batch. Selection authorizes a fresh staged probe, not a bank run. |
