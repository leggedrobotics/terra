...
| `R-20260727-35` | Rejected after exact null | Replacing counterfactual dump/cabin wrapping with raw real transitions preserved outputs but did not improve warmed throughput (`0.9986x` p50; p95 slightly worse), showing XLA already removed the unused work. Revert the extra semantic proof surface. Next test only fixed service batch sizes `4/8/16` with exact output parity before considering another algorithmic change. |
