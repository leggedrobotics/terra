...
| `R-20260727-65` | R-58 v2 passed; one fixed R-60 profile authorized | The corrected four-process cohort completed under the frozen treatment. Receipt `/home/lorenzo/moleworks/.artifacts/terra_b0a_direct_service_cpu_process_probe_20260727_v2/cpu_process_probe.json`, SHA-256 `7a42ecca04100f410afd19d4d2dd25e4a163ccd1797c9faf860350ef7af0aa22`, records four distinct fixed-affinity CPU workers and eight confirmation-identical typed outcomes/counter sets. Cold `C_i` values are `1,353.848/1,477.051/1,644.589/1,776.767 s`; warm `W_i` values are `1,050.778/1,125.202/1,204.426/1,227.335 s`. The preregistered projection gives worst-worker `P_256 = 79,098.861 s` (`21.972 h`) and `P_448 = 138,010.933 s` (`38.336 h`), below 24/48 hours. Conservative aggregate peak RSS is `14,435,036/98,578,360 KiB` (`14.643%`), with zero swap and zero cgroup OOM delta. Before pinning, direct read-only adjudication re-hashed the complete probe artifact inventory, recomputed the typed equality, projection, resource, and authority invariants, and verified all 3,392 frozen B0a manifest entries with `sha256sum --quiet -c files.sha256`. Pin this receipt into the already preregistered R-60 implementation and permit exactly one deterministic four-worker 256-identity profile in a fresh directory. This consumes only `authorizes_one_deterministic_four_worker_256_profile: true`; `authorizes_bank_profile`, retry, alternate widths/backends/kernels, Static, admission, witness, and PPO remain false. |
