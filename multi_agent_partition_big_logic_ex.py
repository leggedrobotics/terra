...
class DisjointMapEnvironments:
    ...
    #     Shows ALL active excavators simultaneously on the global map.
    #     """
    #     try:
    #         self.rng, reset_key = jax.random.split(self.rng)
    ...
    #                         angle_val = float(small_angle.flatten()[0])
    #                     else:
    #                         angle_val = 0.0
    #                 else:
    #                     angle_val = float(small_angle)
    ...
    #                 all_agent_angles.append(angle_val)
                
    #                 print(f"Agent {partition_idx} at global position: {global_pos}, angle: {angle_val}")

    #         # If we have active agents, use the first one's position for the reset
    #         # The rendering system will handle displaying all agents
    ...
    #         self.global_env.all_agent_angles = all_agent_angles
    ...
    # def render_global_environment_with_multiple_agents(self):
    #     """
    #     Render the global environment showing all active excavators.
    #     """
    #     try:
    #         obs = self.global_timestep.observation
    #         info = self.global_timestep.info
    ...
    #         if hasattr(self.global_env, 'all_agent_positions') and hasattr(self.global_env, 'all_agent_angles'):
    #             # Add all agent positions to the info for rendering
    #             info['additional_agents'] = {
    #                 'positions': self.global_env.all_agent_positions,
    #                 'angles': self.global_env.all_agent_angles
    #             }
        ...
        try:
            ...
            for partition_idx, partition_state in partition_states.items():
                if partition_state['status'] == 'active' and partition_state['timestep'] is not None:
                    ...
                    # Handle angle extraction
                    if hasattr(small_angle, 'shape'):
                        if small_angle.shape == ():
                            angle_val = float(small_angle)
                        elif len(small_angle.shape) >= 1:
                            angle_val = float(small_angle.flatten()[0])
                        else:
                            angle_val = 0.0
                    else:
...
def run_experiment_with_disjoint_environments(
    ...
    while playing and step < num_timesteps:
        ...
        #, logits_pi = shared_model.apply(shared_model_params, obs)
        ...
        try:
            ...
            env_manager.partitions[current_partition_idx]['status'] = 'failed'
            partition_state['status'] = 'failed'
            if current_partition_idx in active_partitions:
                active_partitions.remove(current_partition_idx)
        ...
        # This is the key change: render ALL agents simultaneously
