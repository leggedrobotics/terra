...
if __name__ == "__main__":
    ...
    for batch_size in batch_sizes:
        ...
        benchmark_dict["avg_step_env"].append((duration) / (episode_length * batch_size))

        print(f"Duration = {benchmark_dict['time'][-1]}")
        print(f"Average step duration per batch = {benchmark_dict['avg_step_batch'][-1]}")
        print(f"Average step duration per environment = {benchmark_dict['avg_step_env'][-1]}")
