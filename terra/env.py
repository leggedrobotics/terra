...
class TerraEnv(NamedTuple):
    ...
    def step(
        ...
        # Recompute reachability only when the flood-fill seed or terrain changed:
        # - effective macro movement changed the acting agent pose
        # - successful DO changed terrain
        is_do = action.action[0] == TrackedActionType.DO
        is_macro_move = action.action[0] == TrackedActionType.MOVE_TO_POSE
        terrain_changed = jnp.any(new_state.world.action_map.map != state.world.action_map.map)
        pose_changed = jnp.any(
            new_state._get_current_agent_state().pos_base
            != state._get_current_agent_state().pos_base
        )
        angle_changed = jnp.any(
            new_state._get_current_agent_state().angle_base
            != state._get_current_agent_state().angle_base
        )
        current_pose_changed = jnp.logical_or(pose_changed, angle_changed)
        update_reachability = jnp.logical_or(
            jnp.logical_and(is_macro_move, current_pose_changed),
            jnp.logical_and(is_do, terrain_changed),
        )
