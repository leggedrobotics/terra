...
class ExactDumpContractTest(unittest.TestCase):
    ...
    def test_dig_and_relift_exclude_the_rotated_base_footprint(self):
        # Cells inside BOTH the true rotated base footprint and the cabin-1
        # workspace at this pose.  The original fixture cells ((32, 25),
        # (32, 26), (33, 25)) were taken from the transposed footprint raster
        # fixed in 566867db; they are workspace cells but not under the base.
        target = np.zeros(self.SHAPE, dtype=np.int8)
        under_base = np.asarray([[28, 25], [29, 26]])
    ...
    def test_dig_relaxes_soil_exactly_once(self):
        # The dig-side relaxation is contained to the accepted dump zone
        # (target > 0), so the staged piles the relift removes sit inside a
        # zone that covers the workspace; the reference relaxations below use
        ...
        containment = state._accepted_dump_mask()
        once = state._apply_local_soil_mechanics(
            raw,
            dig_mask.reshape(self.SHAPE),
            containment_mask=containment,
        )
