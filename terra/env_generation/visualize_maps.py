...
def visualize_maps_recursive(base_folder, map_categories=['images', 'occupancy', 'dumpability']):
    ...
    for category in map_categories:
        ...
        if category_folder.exists():
            ...
            for npy_file in tqdm(npy_files, desc=f"Processing {category} in {base_folder.name}"):
                map_data = np.load(npy_file)
                print(map_data.shape)
                filename = npy_file.stem  # Removes the file extension
                visualize_and_save_map(map_data, image_output_folder, filename, f"{category.capitalize()}: {filename}")
            
            print(f"Visualization complete for {category}. Images saved in {image_output_folder}")
        else:
...
if __name__ == "__main__":
    ...
    visualize_maps_recursive('/home/lorenzo/git/terra_jax/digbench/data/train/terra/foundations', map_categories=['dumpability'])