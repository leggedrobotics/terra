...
def download_city_crops(
    main_folder,
    center_bbox=(47.378177, 47.364622, 8.526535, 8.544894),
    min_size=(20, 20),
    max_size=(100, 100),
    padding_excavation=3,
    padding_navigation=5,
    resolution=1.0,
):
    ...
    utils.preprocess_dataset_fixed_resolution(
        image_folder, metadata_folder, image_resized_folder, resolution, flip=False
    )
    ...
    utils.pad_images_and_update_metadata(
        image_folder, metadata_folder, padding_excavation, (255, 255, 255), save_folder
    )
    # pad images
    ...
    utils.pad_images_and_update_metadata(
        image_folder, metadata_folder, padding_navigation, (220, 220, 220), save_folder
    )
    ...
    # copy metadata
    utils.copy_metadata(
        main_folder + "/crops_padded_v2", main_folder + "/exterior_crops/metadata"
    )
    # make occupancy, in this case is the same as the images folder
    # copy folder but change name
    shutil.copytree(
        main_folder + "/exterior_crops/images",
        main_folder + "/exterior_crops/occupancy",
        dirs_exist_ok=True,
    )
...
def create_city_crops(main_folder):
    ...
    # copy metadata
    utils.copy_metadata(
        main_folder + "/exterior_crops/metadata", main_folder + "/crops/metadata"
    )
