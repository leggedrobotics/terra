...
from terra.env_generation.create_train_data import (
    ...
    create_procedural_trenches_with_dumpzones,
...
def generate_complete_dataset(config_path="config/env_generation/config.yml", 
                             ...
                             generate_trenches_dumpzones=False,
    ...
    # === TRENCH MAPS WITH DUMP ZONES ===
    if generate_trenches_dumpzones:
        print(f"Step {step_counter}: Creating procedural trenches with dump zones...")
        step_counter += 1
        create_procedural_trenches_with_dumpzones(config)
        print("  ✓ Trench maps with dump zones saved to: data/terra/trenches/*_dumpzone/")
    ...
    if generate_trenches_dumpzones: map_types_to_convert.append("trenches_dumpzone")
    ...
    if generate_terra_format:
        ...
        for size in sizes:
            ...
            # Use the dedicated function for trenches_dumpzone if it's in the map types
            if "trenches_dumpzone" in map_types_to_convert:
                print(f"  Converting trenches_dumpzone with size {size}...")
                convert_to_terra.generate_trenches_dumpzone_terra(npy_dataset_folder, size, n_imgs)
                # Remove trenches_dumpzone from the list to avoid double conversion
                map_types_to_convert = [mt for mt in map_types_to_convert if mt != "trenches_dumpzone"]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--trenches-dumpzones", 
        action="store_true", 
        help="Generate trench maps with specific dump zones"
    )
    parser.add_argument(
        "--relocations", 
        action="store_true", 
    ...
    if args.all or not any([args.foundations, args.foundations_dumpzones, args.foundations_dumpzones_harder, args.trenches, args.trenches_dumpzones, args.relocations, args.relocations_easy, args.relocations_medium, args.relocations_hard, args.relocations_harder, args.terra_format, args.no_terra_format]):
        # Generate everything (default behavior)
        generate_foundations = True
        generate_foundations_dumpzones = False  # Not generated by default
        generate_trenches = True
        generate_trenches_dumpzones = False  # Not generated by default
        generate_relocations = True
        ...
        print("(Use --trenches-dumpzones to also generate dump zone trenches)")
    else:
        ...
        generate_trenches_dumpzones = args.trenches_dumpzones
        ...
        if args.no_terra_format:
            ...
            generate_terra_format = True
        elif any([args.foundations, args.foundations_dumpzones, args.foundations_dumpzones_harder, args.trenches, args.trenches_dumpzones, args.relocations, args.relocations_easy, args.relocations_medium, args.relocations_hard, args.relocations_harder]):
        ...
        if generate_trenches_dumpzones: selected.append("dump zone trenches")
    ...
    generate_complete_dataset(
        ...
        generate_trenches_dumpzones=generate_trenches_dumpzones,
