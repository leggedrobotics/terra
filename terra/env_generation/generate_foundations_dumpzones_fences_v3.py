...
# --- Begin Fences Placement Utility Functions ---
import queue
...
def has_minimum_gap(mask, targets, min_gap):
    ...
    h, w = mask.shape
    # Erode the obstacle mask, then invert to get free-space corridor of at least min_gap
    eroded_obstacles = binary_erosion(mask, structure=np.ones((min_gap, min_gap)))
    eroded_free = ~eroded_obstacles
    ...
    for _ in range(max_trials):
        ...
        if has_minimum_gap(mask_proposed, dump_mask, min_gap) and has_minimum_gap(mask_proposed, found_mask, min_gap):
            ...
            if len(fences) == n_fence:
                break
    return fences
# --- End Fences Placement Utility Functions ---

# Define package directory at module level
PACKAGE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

name_string = "foundations_dumpzones_v3_fence"
...
def create_single_dump_zone(img_terra_pad, size_dump_min, size_dump_max, foundation_mask, min_distance_from_dig: int = 5):
    ...
    # Choose borders farthest from occupied (foundation or buffered dig)
    occ = np.where(forbidden_mask)
    if len(occ[0]) > 0:
        top_gap = occ[0].min()
        bottom_gap = height - 1 - occ[0].max()
        left_gap = occ[1].min()
        right_gap = width - 1 - occ[1].max()
    else:
        # No occupied → all borders equally distant
        top_gap = bottom_gap = left_gap = right_gap = min(height, width)
    border_scores = [
        (0, top_gap),     # 0 = top
        (1, bottom_gap),  # 1 = bottom
        (2, left_gap),    # 2 = left
        (3, right_gap),   # 3 = right
    ]
    # Sort by descending distance, randomize ties a bit for variation
    random.shuffle(border_scores)
    border_scores.sort(key=lambda x: x[1], reverse=True)
    preferred_borders = [b for b, _ in border_scores]

    def sample_xy_for_border(b, s):
        if b == 0:  # top
            x = random.randint(border_offset, width - s - border_offset)
            y = border_offset
        elif b == 1:  # bottom
            x = random.randint(border_offset, width - s - border_offset)
            y = height - s - border_offset
        elif b == 2:  # left
            x = border_offset
            y = random.randint(border_offset, height - s - border_offset)
        else:  # right
            x = width - s - border_offset
            y = random.randint(border_offset, height - s - border_offset)
        return x, y

    # Try preferred borders first, then any remaining
    all_borders = preferred_borders + [b for b in [0,1,2,3] if b not in preferred_borders]

    for attempt in range(max_attempts):
        dump_size = random.randint(size_dump_min, size_dump_max)
        # Cycle through borders in preferred order per attempt window
        border_choice = all_borders[attempt % len(all_borders)]
        x, y = sample_xy_for_border(border_choice, dump_size)
...
def create_foundations_separated_standalone(
    ...
    downsampling_factors = { save_folder: 2.0 }
    ...
    for curriculum_level, downsampling_factor in downsampling_factors.items():
        for i, fn in enumerate(os.listdir(img_folder)):
            ...
            img_terra_pad, dump_cumulative_mask = create_single_dump_zone(
                img_terra_pad, size_dump_min, size_dump_max, foundation_mask, min_distance_from_dig=12
            )

            # Only one foundation dig zone: skip logic for shifting and placing a second foundation shape
            ...
            if np.any(dig_mask):
                ...
                h, w = img_terra_pad.shape[:2]
                # Optionally, allow a simple random translation for variety (not needed for unique second shape)
            ...
            # After placing dig zone and dump zone, add fences ensuring access gap remains
            # --- Compose binary masks for foundation (dig), dump, and existing obstacles ---
            h, w = img_terra_pad.shape[:2]
            foundation_mask = np.all(img_terra_pad == color_dict["digging"], axis=-1)
            dump_mask = np.all(img_terra_pad == color_dict["dumping"], axis=-1)
            obstacle_mask = np.all(img_terra_pad == color_dict["obstacle"], axis=-1)
            forbidden_masks = [foundation_mask, dump_mask, obstacle_mask]
            # FENCE PLACEMENT PARAMS (reverted to working defaults)
            n_fence = random.randint(1, 2)
            fence_width = 2
            fence_minlen = max(12, int(0.4 * max_size))
            fence_maxlen = max(20, int(0.6 * max_size))
            min_gap = 10
            margin = 3
            ...
            fence_final_mask = np.zeros((max_size, max_size), dtype=bool)
            for bx0, by0, bx1, by1 in fences:
                fence_final_mask[by0:by1, bx0:bx1] = True
            img_terra_pad[fence_final_mask] = color_dict["obstacle"]
            # Write occupancy with obstacles only on white background
            occ_img = np.ones((max_size, max_size, 3), dtype=np.uint8) * 255
            occ_img[fence_final_mask] = color_dict["obstacle"]
            cv2.imwrite(str(Path(curriculum_level, "occupancy", f"trench_{n}.png")), occ_img)
            # (If you also have an occupancy or dumpability map later, update them too)
            ...
            # Write to occupancy image as well: white background, blue on obstacles only
            occ_img = np.ones((max_size, max_size, 3), dtype=np.uint8) * 255  # white
            occ_img[fence_final_mask] = color_dict["obstacle"]
