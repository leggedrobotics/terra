...
class State(NamedTuple):
    ...
    def _get_rewards_truck(self, new_state: "State", action: ActionType) -> Float:
        ...
        # Proximity reward for loaded truck: encourage moving toward dump zones
        is_loaded = cur.loaded[0] > 0
        
        def _dump_zone_proximity_term():
            # Get truck positions (old and new)
            old_x = cur.pos_base[0].astype(jnp.float32)
            old_y = cur.pos_base[1].astype(jnp.float32)
            new_agent_state = new_state._get_prev_agent_state()
            new_x = new_agent_state.pos_base[0].astype(jnp.float32)
            ...
            # Find dump zones (target_map > 0) and compute Euclidean distance to all dump zone tiles
            dump_zone_mask = self.world.target_map.map > 0
            dump_coords = jnp.argwhere(dump_zone_mask)
            has_dump_tiles = dump_coords.shape[0] > 0

            def _first_dump_coord():
                coord = dump_coords[0].astype(jnp.float32)
                return coord[0], coord[1]

            ref_x, ref_y = jax.lax.cond(
                has_dump_tiles,
                _first_dump_coord,
                lambda: (jnp.float32(0.0), jnp.float32(0.0)),
            )

            def _distance(px: Float, py: Float) -> Float:
                dx = ref_x - px
                dy = ref_y - py
                return jnp.sqrt(dx * dx + dy * dy)

            min_dist_old = jax.lax.cond(
                has_dump_tiles,
                lambda: _distance(old_x, old_y),
                lambda: jnp.float32(1e9),
            )

            min_dist_new = jax.lax.cond(
                has_dump_tiles,
                lambda: _distance(new_x, new_y),
                lambda: jnp.float32(1e9),
            )
            
            # Reward for moving closer to dump zone (similar to proximity reward for excavator)
            close_bonus = jnp.float32(0.15)
            retreat_penalty = jnp.float32(-0.15)
            delta_reward = jnp.where(
                min_dist_new < min_dist_old,
                close_bonus,
                jnp.where(min_dist_new > min_dist_old, retreat_penalty, jnp.float32(0.0)),
            )
            
            return delta_reward
        
        reward += jax.lax.cond(jnp.logical_and(is_truck, is_loaded), _dump_zone_proximity_term, lambda: 0.0)
        return reward
