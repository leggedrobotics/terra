...
def load_maps_from_disk(folder_path: str) -> Array:
    ...
    for i in tqdm(selected_indices, desc="Data Loader"):
        ...
        try:
            ...
            n_loaded_metadata += 1
        except (FileNotFoundError, json.JSONDecodeError, RuntimeError, KeyError, TypeError) as e:
            n_missing_metadata += 1
            if first_metadata_error is None:
                first_metadata_error = (
                    f"map index {i} ({folder_path}/metadata/trench_{i}.json): {e}"
                )
    ...
    if n_loaded_metadata > 0:
        ...
        if n_missing_metadata > 0:
            print(
                f"Warning: {n_missing_metadata} maps in {folder_path} had missing or invalid "
                "metadata; dummy axes were used for those maps. "
                f"First failure: {first_metadata_error}"
            )
    else:
