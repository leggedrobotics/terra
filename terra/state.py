...
class State(NamedTuple):
    ...
    def new(
        ...
        randomize_env_steps: bool = False,
    ) -> "State":
        ...
        if randomize_env_steps:
            key, step_key = jax.random.split(key)
            max_steps = jnp.maximum(jnp.asarray(env_cfg.max_steps_in_episode), 1)
            env_steps = jax.random.randint(
                step_key,
                (),
                minval=0,
                maxval=max_steps,
                dtype=jnp.asarray(env_cfg.max_steps_in_episode).dtype,
            )
        else:
            env_steps = jnp.asarray(0, dtype=jnp.asarray(env_cfg.max_steps_in_episode).dtype)
    ...
    def _step(self, action: Action, turn:bool = True) -> "State":
        ...
        state = jax.lax.cond(
            ...
            self._do_nothing,
            lambda: jax.lax.cond(
                action_idx == TrackedActionType.SET_BASE_ANGLE,
                lambda: self._handle_macro_set_base_angle(action.target_pose[2]),
                lambda: jax.lax.cond(
                    action_idx == TrackedActionType.SET_CABIN_ANGLE,
                    lambda: self._handle_macro_set_cabin_angle(action.target_pose[3]),
                    lambda: jax.lax.cond(
                        action_idx == TrackedActionType.MOVE_TO_POSE,
                        lambda: self._handle_macro_move_to_pose(action.target_pose),
    ...
    def _inflate_blocked(blocked: Array, radius: int) -> Array:
        ...
        return jax.lax.fori_loop(0, radius, lambda _, x: _dilate_once(x), blocked)
    ...
    def _agent_footprint_mask(self, agent_state: AgentState) -> Array:
    ...
    def _other_agent_footprint_mask(self) -> Array:
        def _zero_mask():
            return jnp.zeros((self.world.height, self.world.width), dtype=jnp.bool_)

        def _mask_for_idx(i):
            st = self.agent.agent_states[i]
            include = jnp.logical_and(self.agent.agent_active[i] == 1, i != self.agent.current_agent)
            return jax.lax.cond(include, lambda: self._agent_footprint_mask(st), _zero_mask)

        mask0 = _mask_for_idx(0)
        mask1 = _mask_for_idx(1)
        mask2 = _mask_for_idx(2)
        mask3 = _mask_for_idx(3)
        return jnp.logical_or(jnp.logical_or(mask0, mask1), jnp.logical_or(mask2, mask3))

    def _macro_reachability_mask(self) -> Array:
        cur = self._get_current_agent_state()
        start_mask = self._agent_footprint_mask(cur)
        other_agents = self._other_agent_footprint_mask()
        traversability = self._build_traversability_mask(
            self.world.action_map.map,
            self.world.static_traversability_base.map,
        ).astype(jnp.bool_)
        blocked = jnp.logical_or(traversability, other_agents)
        inflated = self._inflate_blocked(
            blocked,
            getattr(self.env_cfg, "macro_move_reachability_inflation_tiles", 2),
        )
        passable = jnp.logical_not(inflated)
        return self._build_reachability_mask(passable, start_mask)

    def _is_macro_target_reachable(self, target_pos: Array) -> Array:
        reachability = self._macro_reachability_mask()
        target_x = jnp.clip(target_pos[0], 0, self.world.height - 1)
        target_y = jnp.clip(target_pos[1], 0, self.world.width - 1)
        return reachability[target_x, target_y] > 0

    def _can_macro_move_while_loaded(self) -> Array:
        cur = self._get_current_agent_state()
        is_loaded = cur.loaded[0] > 0
        is_truck = cur.agent_type[0] == 1
        is_skid_steer = cur.agent_type[0] == 2
        can_carry = jnp.logical_or(is_truck, is_skid_steer)
        return jnp.logical_or(can_carry, jnp.logical_not(is_loaded))

    def _handle_macro_move_to_pose(self, target_pose: Array) -> "State":
        cur = self._get_current_agent_state()
        target_pos = target_pose[:2].astype(IntMap)
        target_angle = jnp.mod(
            target_pose[2],
            jnp.array(self.env_cfg.agent.angles_base, dtype=IntMap),
        ).astype(IntLowDim)
        target_angle = jnp.reshape(target_angle, cur.angle_base.shape)
        target_cabin_angle = jnp.mod(
            target_pose[3],
            jnp.array(self.env_cfg.agent.angles_cabin, dtype=IntMap),
        ).astype(IntLowDim)
        target_cabin_angle = jnp.reshape(target_cabin_angle, cur.angle_cabin.shape)

        agent_corners = self._get_agent_corners(
            target_pos,
            base_orientation=target_angle,
            agent_width=self.env_cfg.agent.width,
            agent_height=self.env_cfg.agent.height,
        )
        valid_footprint = self._is_valid_move(agent_corners)
        reachable = self._is_macro_target_reachable(target_pos)
        can_move_loaded = self._can_macro_move_while_loaded()
        valid_move = jnp.logical_and(
            can_move_loaded,
            jnp.logical_and(valid_footprint, reachable),
        )

        new_pos = jnp.where(valid_move, target_pos, cur.pos_base)
        new_angle = jnp.where(valid_move, target_angle, cur.angle_base)
        can_set_cabin = jnp.logical_not(
            jnp.logical_or(cur.agent_type[0] == 1, cur.agent_type[0] == 2)
        )
        new_cabin_angle = jnp.where(
            jnp.logical_and(valid_move, can_set_cabin),
            target_cabin_angle,
            cur.angle_cabin,
        )
        new_wheel_angle = jnp.where(
            valid_move,
            jnp.zeros_like(cur.wheel_angle),
            cur.wheel_angle,
        )
        return self._set_current_agent_state(
            cur._replace(
                pos_base=new_pos,
                angle_base=new_angle,
                angle_cabin=new_cabin_angle,
                wheel_angle=new_wheel_angle,
            )
        )

    def _handle_macro_set_cabin_angle(self, target_cabin_angle: Array) -> "State":
        cur = self._get_current_agent_state()
        is_skid_steer = cur.agent_type[0] == 2
        is_truck = cur.agent_type[0] == 1

        def _set_cabin_angle():
            cur2 = self._get_current_agent_state()
            target_angle = jnp.mod(
                target_cabin_angle,
                jnp.array(self.env_cfg.agent.angles_cabin, dtype=IntMap),
            ).astype(IntLowDim)
            target_angle = jnp.reshape(target_angle, cur2.angle_cabin.shape)
            target_angle = self._apply_cabin_rotation_mask(
                cur2.angle_cabin,
                target_angle,
            )
            return self._set_current_agent_state(
                cur2._replace(angle_cabin=target_angle)
            )
    ...
    def _handle_macro_set_base_angle(self, target_base_angle: Array) -> "State":
        cur = self._get_current_agent_state()
        ...
        target_angle = jnp.reshape(target_angle, cur.angle_base.shape)
        target_angle = self._apply_base_rotation_mask(cur.angle_base, target_angle)

        angle_changed = ~jnp.all(cur.angle_base == target_angle)
        new_wheel_angle = jnp.where(
            angle_changed,
            jnp.zeros_like(cur.wheel_angle),
            cur.wheel_angle,
    ...
    def _skid_steer_auto_load_dirt(self, new_state: "State") -> "State":
        ...
        # Use configured skid steer capacity (fallback to 127 for backward compatibility)
        workspace_capacity = jnp.int32(getattr(self.env_cfg, 'skidsteer_capacity', 127))
    ...
    def _get_foundation_border_alignment_mask(self, workspace_mask_flat: Array) -> Array:
        ...
        def _metadata_alignment_mask():
            ...
            def _check_active_segment(i, carry):
                ...
                tol = jnp.where(is_hv_edge, hv_tol, diag_tol)

                diff = jnp.abs(wrap_angle_rad(edge_angle - arm_angle))
                diff = jnp.minimum(diff, jnp.pi - diff)
                aligned = diff <= tol
            ...
            corner_allowed_mask = jnp.zeros((rows, cols), dtype=jnp.bool_)

            def _collect_corner_mask(i, mask):
                next_i = jnp.where(i + 1 < border_type, i + 1, 0)
                abc1 = self.world.foundation_border_axes[i]
                abc2 = self.world.foundation_border_axes[next_i]
                valid = jnp.logical_and(i < border_type, border_type > 1)
                det = abc1[0] * abc2[1] - abc2[0] * abc1[1]
                valid = jnp.logical_and(valid, jnp.abs(det) > 1e-6)
                x = (abc1[1] * abc2[2] - abc2[1] * abc1[2]) / det
                y = (abc1[2] * abc2[0] - abc2[2] * abc1[0]) / det
                dist2 = (cc - x) * (cc - x) + (rr - y) * (rr - y)
                near_corner = jnp.logical_and(valid, dist2 <= corner_thr_tiles * corner_thr_tiles)
                pair_allowed = jnp.logical_or(allowed_segments[i], allowed_segments[next_i])
                return jnp.logical_or(mask, jnp.logical_and(near_corner, pair_allowed))

            corner_allowed_mask = jax.lax.fori_loop(
                0,
                max_border_axes,
                _collect_corner_mask,
                corner_allowed_mask,
            )
            ...
            def _collect_allowed_tile_segment(i, carry):
                ...
                return jnp.logical_or(carry, jnp.logical_and(best_idx == i, allowed_i))

            nearest_allowed = jax.lax.fori_loop(
                0,
                max_border_axes,
        ...
        def _fallback_alignment_mask():
            ...
            return jnp.logical_or(jnp.logical_not(border_mask), aligned).reshape(-1)
    ...
    def _get_dig_dump_mask_cyl(self, map_cyl_coords: Array) -> Array:
        ...
        min_distance_from_agent = tile_size * max_agent_dim #in meters
    ...
    def _dilate_mask(mask: Array, kernel_size: int = 3) -> Array:
        """Dilate a 2D mask or a batched [B,H,W] mask with a square window."""
        mask_f = mask.astype(jnp.float32)
        if mask.ndim == 2:
            ...
            raise ValueError(f"Unsupported mask rank {mask.ndim}; expected 2D or 3D.")
        return (
    ...
    def _truck_allowed_tiles_mask(self) -> Array:
        ...
        def _road_restricted_mask():
            ...
            dilated = self._dilate_mask(base_allowed, kernel_size=3)
            return dilated
    ...
    def _handle_rewards_macro_move(
        self, new_state: "State", action: TrackedActionType
    ) -> Float:
        old_pos = self._get_current_agent_state().pos_base.astype(jnp.float32)
        new_pos = new_state._get_next_agent_state().pos_base.astype(jnp.float32)
        moved = ~jnp.allclose(old_pos, new_pos)
        path_length_tiles = jnp.sum(jnp.abs(new_pos - old_pos))
        move_cost = -jnp.float32(getattr(self.env_cfg, "macro_move_cost_per_tile", 0.02)) * path_length_tiles

        def _successful_macro_move_reward():
            old_dump_dist = self._min_distance_to_dump_zone(old_pos)
            new_dump_dist = self._min_distance_to_dump_zone(new_pos)
            closer_tiles = jnp.maximum(old_dump_dist - new_dump_dist, 0.0)
            dumpzone_bonus = jnp.minimum(
                closer_tiles
                * jnp.float32(getattr(self.env_cfg, "macro_move_dumpzone_closer_reward", 0.01)),
                jnp.float32(getattr(self.env_cfg, "macro_move_dumpzone_closer_reward_cap", 0.2)),
            )
            has_dumpzone = jnp.any(self.world.target_map.map > 0)
            return move_cost + jnp.where(has_dumpzone, dumpzone_bonus, jnp.float32(0.0))

        return jax.lax.cond(
            moved,
            _successful_macro_move_reward,
            lambda: jnp.float32(self.env_cfg.rewards.collision_move),
        )
    ...
    def _handle_rewards_macro_cabin_angle(
        self, new_state: "State", action: TrackedActionType
    ) -> Float:
        old_angle = self._get_current_agent_state().angle_cabin
        new_angle = new_state._get_next_agent_state().angle_cabin
        changed = ~jnp.all(old_angle == new_angle)

        def _changed_reward():
            acted_state = new_state._replace(
                agent=new_state.agent._replace(current_agent=self.agent.current_agent)
            )
            workspace_mask = acted_state._build_dig_dump_cone().reshape(
                acted_state.world.action_map.map.shape
            )
            remaining_dig = jnp.logical_and(
                acted_state.world.target_map.map < 0,
                acted_state.world.action_map.map > acted_state.world.target_map.map,
            )
            has_remaining_dig_in_workspace = jnp.any(
                jnp.logical_and(workspace_mask, remaining_dig)
            )
            return jnp.where(
                has_remaining_dig_in_workspace,
                jnp.float32(getattr(self.env_cfg, "macro_cabin_workspace_dig_reward", 0.05)),
                jnp.float32(0.0),
            )
    ...
    def _handle_rewards_macro_base_angle(
        self, new_state: "State", action: TrackedActionType
    ) -> Float:
        old_angle = self._get_current_agent_state().angle_base
        new_angle = new_state._get_next_agent_state().angle_base
        changed = ~jnp.all(old_angle == new_angle)
        return jax.lax.cond(
            changed,
            lambda: jnp.float32(0.0),
    ...
    def _get_rewards_excavator(self, new_state: "State", action: ActionType) -> Float:
        ...
        reward += jax.lax.cond(
            action == TrackedActionType.MOVE_TO_POSE,
            self._handle_rewards_macro_move,
            lambda new_state, action: 0.0,
            new_state,
            action,
        )
        ...
        reward += jax.lax.cond(
            action == TrackedActionType.SET_CABIN_ANGLE,
            self._handle_rewards_macro_cabin_angle,
            lambda new_state, action: 0.0,
            new_state,
            action,
        )
        ...
        reward += jax.lax.cond(
            action == TrackedActionType.SET_BASE_ANGLE,
            self._handle_rewards_macro_base_angle,
            lambda new_state, action: 0.0,
            new_state,
            action,
        )
    ...
    def _get_reward(self, new_state: "State", action_handler: Action):
        ...
        path_length_tiles = jnp.sum(jnp.abs(new_pos - old_pos))
        moved = ~jnp.allclose(old_pos, new_pos)
        is_macro_move = action[0] == TrackedActionType.MOVE_TO_POSE
        components["move_meters"] = jnp.where(
            jnp.logical_and(is_macro_move, moved),
            path_length_tiles * self.env_cfg.tile_size,
            jnp.float32(0.0),
        )
        components["macro_move_count"] = jnp.where(
            is_macro_move,
            jnp.float32(1.0),
            jnp.float32(0.0),
        )
        ...
        gated_completion = jax.lax.cond(
            enforce_border_alignment,
            lambda: self._calculate_edge_inner_min_completion(action_map, edge_mask),
    ...
    def _get_action_mask_tracked(self):
        ...
        cur = self._get_current_agent_state()
        bool_set_cabin_angle = ~jnp.logical_or(cur.agent_type[0] == 1, cur.agent_type[0] == 2)
        bool_move_to_pose = self._can_macro_move_while_loaded()
        bool_set_base_angle = True
    ...
    def _calculate_edge_inner_min_completion(
        self, action_map: Array, border_mask: Array | None = None
    ) -> Float:
        """
