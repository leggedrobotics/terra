...
class DisjointMapEnvironments:
    ...
    def _update_global_environment_display_with_all_agents(self, partition_states):
        ...
        try:
            ...
            import traceback
            traceback.print_exc()
    # Add these methods to the DisjointMapEnvironments class

    def _get_overlapping_regions(self, partition_idx):
        """
        Get all partitions that overlap with the given partition.
        Returns a list of partition indices that share territory.
        """
        current_partition = self.partitions[partition_idx]
        y1, x1, y2, x2 = current_partition['region_coords']
        
        overlapping = []
        for i, other_partition in enumerate(self.partitions):
            if i == partition_idx:
                continue
                
            oy1, ox1, oy2, ox2 = other_partition['region_coords']
    ...
    def _get_overlap_region(self, partition1_coords, partition2_coords):
        """
        Calculate the overlapping region between two partitions.
        Returns the coordinates of the overlap region.
        """
        y1, x1, y2, x2 = partition1_coords
        oy1, ox1, oy2, ox2 = partition2_coords
        ...
        if overlap_y1 <= overlap_y2 and overlap_x1 <= overlap_x2:
            return (overlap_y1, overlap_x1, overlap_y2, overlap_x2)
        else:
            return None

    def _resolve_overlapping_updates(self, partition_states, map_name):
        """
        Resolve conflicts in overlapping regions by using a priority system.
        Higher priority partitions (lower index) take precedence in overlap regions.
        """
        # Create a copy of the global map to work with
        updated_map = self.global_maps[map_name].copy()
        
        # Track which pixels have been updated and by which partition
        update_mask = jnp.zeros_like(updated_map, dtype=jnp.int32) - 1  # -1 means not updated
        
        # Process partitions in order (lower index = higher priority)
        for partition_idx in sorted(partition_states.keys()):
            ...
            # Get the updated map from this partition
            small_state = partition_state['timestep'].state
            small_map = getattr(small_state.world, map_name).map
            
            # Extract the region size
            region_height = y_end - y_start + 1
            region_width = x_end - x_start + 1
            extract_height = min(region_height, small_map.shape[0])
            extract_width = min(region_width, small_map.shape[1])
            
            extracted_region = small_map[:extract_height, :extract_width]
            
            # For overlapping regions, only update pixels that haven't been updated by higher priority partitions
            for y in range(extract_height):
                for x in range(extract_width):
                    ...
                    if (global_y < updated_map.shape[0] and global_x < updated_map.shape[1]):
                        # Only update if not already updated by a higher priority partition
                        if update_mask[global_y, global_x] == -1:
                            updated_map = updated_map.at[global_y, global_x].set(extracted_region[y, x])
                            update_mask = update_mask.at[global_y, global_x].set(partition_idx)
        
        return updated_map

    def _merge_overlapping_updates(self, partition_states, map_name):
        """
        Alternative approach: merge updates from overlapping regions using averaging.
        This preserves contributions from all partitions.
        """
        # Create accumulator arrays
        accumulator = jnp.zeros_like(self.global_maps[map_name], dtype=jnp.float32)
        count_mask = jnp.zeros_like(self.global_maps[map_name], dtype=jnp.int32)
        
        # Accumulate updates from all partitions
        for partition_idx, partition_state in partition_states.items():
            if partition_state['status'] != 'active' or partition_state['timestep'] is None:
                continue
            ...
            extracted_region = small_map[:extract_height, :extract_width].astype(jnp.float32)
            
            # Add to accumulator
            region_slice = (slice(y_start, y_start + extract_height), 
                        slice(x_start, x_start + extract_width))
            
            accumulator = accumulator.at[region_slice].add(extracted_region)
            count_mask = count_mask.at[region_slice].add(1)
        
        # Average the accumulated values
        # Avoid division by zero
        safe_count = jnp.where(count_mask > 0, count_mask, 1)
        averaged_map = accumulator / safe_count
        
        # Only update regions that were actually modified
        final_map = jnp.where(count_mask > 0, averaged_map, self.global_maps[map_name])
        
        return final_map

    def update_global_maps_from_all_small_environments(self, partition_states, merge_strategy='priority'):
        """
        Update global maps with changes from ALL active small environments.
        Enhanced to handle overlapping partitions using different merge strategies.
        
        Args:
            partition_states: Dictionary of partition states
            merge_strategy: 'priority' (higher priority wins) or 'average' (average overlapping regions)
        """
        # List of maps that need special handling for overlaps
        overlap_sensitive_maps = ['dumpability_mask', 'target_map', 'action_map', 'dig_map']
        
        for map_name in ['dumpability_mask', 'target_map', 'action_map', 'traversability_mask', 
                        'padding_mask', 'dig_map']:
            
            if map_name in overlap_sensitive_maps:
                # Use special handling for overlap-sensitive maps
                if merge_strategy == 'priority':
                    self.global_maps[map_name] = self._resolve_overlapping_updates(partition_states, map_name)
                elif merge_strategy == 'average':
                    self.global_maps[map_name] = self._merge_overlapping_updates(partition_states, map_name)
                else:
                    raise ValueError(f"Unknown merge strategy: {merge_strategy}")
            else:
                # Use original method for non-overlap sensitive maps
                self._update_single_map_original(partition_states, map_name)

    def _update_single_map_original(self, partition_states, map_name):
        """
        Original update method for maps that don't need special overlap handling.
        """
        for partition_idx, partition_state in partition_states.items():
            if partition_state['status'] == 'active' and partition_state['timestep'] is not None:
                partition = self.partitions[partition_idx]
                ...
                try:
                    ...
                    print(f"Shape mismatch for {map_name}: {e}")
                    # Handle shape mismatch by padding or cropping
    ...
    def validate_partition_overlap(self):
        """
        Validate that overlapping partitions are properly configured.
        Returns True if configuration is valid, False otherwise.
        """
        for i, partition1 in enumerate(self.partitions):
            for j, partition2 in enumerate(self.partitions[i+1:], i+1):
                overlap = self._get_overlap_region(partition1['region_coords'], partition2['region_coords'])
                if overlap:
                    print(f"Partitions {i} and {j} overlap in region {overlap}")
                    
                    # Check if overlap is too large (more than 50% of either partition)
                    y1, x1, y2, x2 = partition1['region_coords']
                    oy1, ox1, oy2, ox2 = partition2['region_coords']
                    
                    area1 = (y2 - y1 + 1) * (x2 - x1 + 1)
                    area2 = (oy2 - oy1 + 1) * (ox2 - ox1 + 1)
                    
                    overlap_y1, overlap_x1, overlap_y2, overlap_x2 = overlap
                    overlap_area = (overlap_y2 - overlap_y1 + 1) * (overlap_x2 - overlap_x1 + 1)
                    
                    overlap_ratio1 = overlap_area / area1
                    overlap_ratio2 = overlap_area / area2
                    
                    print(f"  Overlap ratio for partition {i}: {overlap_ratio1:.2%}")
                    print(f"  Overlap ratio for partition {j}: {overlap_ratio2:.2%}")
                    
                    if overlap_ratio1 > 0.5 or overlap_ratio2 > 0.5:
                        print(f"  WARNING: Large overlap detected (>50%)")
                        return False
        
        return True

    # Additional utility function to visualize overlaps
    def visualize_partition_overlaps(self):
        """
        Create a visualization of partition overlaps.
        Returns a 2D array where each cell shows which partitions cover it.
        """
        # Determine the size of the global map
        map_height, map_width = self.global_maps['target_map'].shape
        
        # Create an overlap map
        overlap_map = jnp.zeros((map_height, map_width), dtype=jnp.int32)
        
        # Mark each partition's coverage
        for i, partition in enumerate(self.partitions):
            y_start, x_start, y_end, x_end = partition['region_coords']
            
            # Use bit flags to represent multiple partitions
            for y in range(y_start, min(y_end + 1, map_height)):
                for x in range(x_start, min(x_end + 1, map_width)):
                    overlap_map = overlap_map.at[y, x].set(overlap_map[y, x] | (1 << i))
...
def run_experiment_with_disjoint_environments(
    ...
    #             {'id': 3, 'region_coords': (64, 64, 127, 127), 'start_pos': (96, 96), 'start_angle': 0, 'status': 'pending'}
    #         ]
    #overlapping # 1x2 partitioning
    sub_tasks_manual = [
        {'id': 0, 'region_coords': (0, 0, 60, 60), 'start_pos': (25, 20), 'start_angle': 0, 'status': 'pending'},
        {'id': 1, 'region_coords': (40, 0, 100, 60), 'start_pos': (75, 20), 'start_angle': 0, 'status': 'pending'}
    ]
    sub_tasks_llm = []
    ...
    MERGE_STRATEGY = 'priority'  # Options: 'priority' or 'average'
    ENABLE_OVERLAP_VALIDATION = True  # Set to True to validate overlapping partitions
    ...
    # Add validation after setting up the environment manager:
    if ENABLE_OVERLAP_VALIDATION:
        print("Validating partition configuration for overlaps...")
        if not env_manager.validate_partition_overlap():
            print("WARNING: Partition configuration has potential issues!")
            # You can choose to continue or exit here
        else:
            print("Partition configuration validated successfully.")
        
        # Show overlap visualization
        overlap_viz = env_manager.visualize_partition_overlaps()
        unique_overlaps = jnp.unique(overlap_viz)
        print(f"Overlap patterns found: {unique_overlaps}")
        print(f"Regions with overlaps: {jnp.sum(overlap_viz > 1)} pixels")
    ...
    while playing and step < num_timesteps:
        ...
        # Update global maps using overlap-aware strategy
        env_manager.update_global_maps_from_all_small_environments(partition_states, MERGE_STRATEGY)
