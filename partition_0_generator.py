#!/usr/bin/env python3
"""
Script to generate a file from PNG sequence files for PARTITION 0 only.
Processes files with pattern: traversability_mask_partition_0_step_X.png
"""
...
def find_partition_0_files(directory: str) -> List[str]:
    """
    Find all PNG files for partition 0 and sort them by step number.
    ...
    def extract_step_number(filename):
        match = re.search(r'partition_0_step_(\d+)', filename)
        if match:
...
def create_gif_animation(image_files: List[str], output_path: str, duration: int = 500) -> None:
    """Create a GIF animation from partition 0 PNG files."""
    if not image_files:
        print("No partition 0 image files found!")
        return
    ...
    if images:
        ...
        print(f"Partition 0 GIF animation saved to: {output_path}")
        print(f"Total frames: {len(images)}")
    else:
        print("No valid partition 0 images found to create GIF!")

def create_grid_composite(image_files: List[str], output_path: str, cols: int = None) -> None:
    """Create a grid composite from partition 0 PNG files."""
    if not image_files:
        print("No partition 0 image files found!")
        return
    ...
    if not images:
        print("No valid partition 0 images found!")
        return
    ...
    print(f"Partition 0 grid composite saved to: {output_path} ({cols}x{rows} grid)")
    print(f"Total images: {len(images)}")

def create_horizontal_strip(image_files: List[str], output_path: str) -> None:
    """Create a horizontal strip from partition 0 PNG files."""
    if not image_files:
        print("No partition 0 image files found!")
        return
    ...
    composite.save(output_path)
    print(f"Partition 0 horizontal strip saved to: {output_path}")
    print(f"Total images: {len(images)}")

def main():
    parser = argparse.ArgumentParser(description="Generate file from PARTITION 0 PNG sequence")
    ...
    if not png_files:
        print("No partition 0 PNG files found!")
        return
    
    print(f"Found {len(png_files)} partition 0 files:")
    ...
    if args.type == "gif":
        if not args.output.lower().endswith('.gif'):
            args.output = args.output.replace('.', '_partition_0.') if '.' in args.output else args.output + '_partition_0.gif'
